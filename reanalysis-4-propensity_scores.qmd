...
```{r ps-formula}
# Use original formula from get_matching_formula() 
# to maintain consistency with original analysis

ps_formula_original <- get_matching_formula()

```

**Propensity score formula (original, without DR severity):**
```{r print-formula}
# Show the formula being used
print(ps_formula_original)

# Set the formula to use for analysis
...
for (i in 1:n_imputations) {
  ...
  if (use_cache && file.exists(cache_file)) {
    log_message(paste0("Imputation ", i, ": Loading from cache"))
    cached_result <- readRDS(cache_file)
    twang_results[[i]] <- cached_result
    weights <- cached_result$weights
  } else {
    log_message(paste0("Imputation ", i, ": Fitting twang model"))
    imp_start <- Sys.time()
    ...
    log_message(paste0("Imputation ", i, ": Completed in ", imp_time, " minutes, saved to cache"))
  }
  
  # Track progress (works for both cached and newly computed)
  progress_df <- rbind(progress_df, data.frame(
    Imputation = i,
    Trees_Used = twang_results[[i]]$n_trees_used,
    Weight_Min = round(min(weights), 3),
    Weight_Max = round(max(weights), 3)
  ))
  
  # Log progress with time estimates
  log_progress(i, n_imputations, "Imputation")
}
