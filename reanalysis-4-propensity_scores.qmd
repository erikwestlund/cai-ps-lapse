...
log_message(paste0("Parallel processing: Using ", n_cores, " cores"))
...
process_imputation <- function(i, imp_datasets, formula, trees, cache_path, use_caching) {
  ...
  if (use_caching && file.exists(cache_file)) {
    ...
    # Ensure lapse is properly formatted as numeric 0/1
    if ("lapse" %in% names(imp_data)) {
      if (is.factor(imp_data$lapse)) {
        # Convert factor to numeric via character to avoid level issues
        imp_data$lapse <- as.numeric(as.character(imp_data$lapse))
      } else if (is.character(imp_data$lapse)) {
        imp_data$lapse <- as.numeric(imp_data$lapse)
      } else if (is.logical(imp_data$lapse)) {
        imp_data$lapse <- as.numeric(imp_data$lapse)
      }
      
      # Ensure values are 0 or 1
      if (any(!imp_data$lapse %in% c(0, 1, NA))) {
        stop(paste("Lapse variable contains values other than 0, 1, or NA in imputation", i))
      }
    } else {
      stop(paste("Lapse variable not found in imputation", i))
    }
...
if (.Platform$OS.type == "unix") {
  ...
  parallel_results <- mclapply(
    1:n_imputations,
    function(i) process_imputation(i, imputed_datasets, ps_formula, n_trees, cache_dir, use_cache),
    mc.cores = n_cores
  )
} else {
  ...
  parallel_results <- parLapply(cl, 1:n_imputations, function(i) {
    process_imputation(i, imputed_datasets, ps_formula, n_trees, cache_dir, use_cache)
  })
...
for (res in parallel_results) {
  ...
  if (res$cached) {
    log_message(paste0("Imputation ", res$imputation, ": Loaded from cache"))
  } else {
    log_message(paste0("Imputation ", res$imputation, ": Fitted in ", res$time_mins, " minutes"))
  }
}
...
kable(progress_df, 
      ...
      col.names = c("Imputation", "Trees Used", "Min Weight", "Max Weight", "From Cache", "Time (min)"))
```
