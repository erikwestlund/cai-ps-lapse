...
for (i in 1:n_imputations) {
  ...
  tryCatch({
    ...
    # For ATT with es.mean stop method, the balance table is stored in bal_table$es.mean.ATT
    if (!is.null(bal_table$es.mean.ATT) && !is.null(bal_table$es.mean.ATT$std.eff.sz)) {
      # Get the maximum absolute standardized effect size
      largest_smds[i] <- max(abs(bal_table$es.mean.ATT$std.eff.sz), na.rm = TRUE)
    } else if (!is.null(bal_table$es.mean) && !is.null(bal_table$es.mean$std.eff.sz)) {
      # Fallback to es.mean if es.mean.ATT not available
      largest_smds[i] <- max(abs(bal_table$es.mean$std.eff.sz), na.rm = TRUE)
    } else {
      # Try to get from the twang_model summary directly
      largest_smds[i] <- twang_model$desc$es.mean$max.es
    }
  }, error = function(e) {
    # If balance table fails, try to get from model summary
    tryCatch({
      largest_smds[i] <- twang_model$desc$es.mean$max.es
    }, error = function(e2) {
      largest_smds[i] <- NA
...
# Display average
if (!is.na(avg_largest_smd)) {
  cat("\n**Average of largest absolute SMDs across imputations:**", round(avg_largest_smd, 4), "\n")
} else {
  cat("\n**Average of largest absolute SMDs across imputations:** NA (unable to extract SMDs)\n")
}
...
if (!is.na(avg_largest_smd)) {
  ...
  cat("**Interpretation:** Unable to assess balance - SMD extraction failed\n")
}
```
