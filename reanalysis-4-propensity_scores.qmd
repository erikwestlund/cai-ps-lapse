...
max_cores_for_ps <- 1  # Maximum cores to use for propensity score estimation (reduced for memory management)
...
log_message(paste0("Detected ", available_cores, " available cores"))
log_message(paste0("Max cores configured: ", max_cores_for_ps))
...
process_imputation <- function(i, imp_datasets, formula, trees, cache_path, use_caching, worker_id, log_dir) {
  ...
  if (use_caching && file.exists(cache_file)) {
    ...
    # Log cache hit
    cache_msg <- paste0("[Worker ", worker_id, "] Imputation ", i, " loaded from cache")
...
# Create assignments of imputations to workers
# With 2 workers: Worker 1 gets 1-15, Worker 2 gets 16-30
worker_assignments <- data.frame(
  imputation = 1:n_imputations,
  worker_id = rep(1:n_cores, length.out = n_imputations)[1:n_imputations]
)

# For more even distribution with 2 workers
if (n_cores == 2) {
  midpoint <- ceiling(n_imputations / 2)
  worker_assignments$worker_id <- ifelse(worker_assignments$imputation <= midpoint, 1, 2)
}

# Run parallel processing
log_message(paste0("Starting parallel processing of ", n_imputations, " imputations using ", n_cores, " cores"))
log_message(paste0("Worker 1 will process imputations: ", 
                   paste(worker_assignments$imputation[worker_assignments$worker_id == 1], collapse = ", ")))
if (n_cores >= 2) {
  log_message(paste0("Worker 2 will process imputations: ", 
                     paste(worker_assignments$imputation[worker_assignments$worker_id == 2], collapse = ", ")))
}
...
if (.Platform$OS.type == "unix") {
  ...
  parallel_results <- mclapply(
    1:n_imputations,
    function(i) {
      worker_id <- worker_assignments$worker_id[i]
      process_imputation(i, imputed_datasets, ps_formula, n_trees, cache_dir, use_cache, worker_id, log_dir)
    },
    mc.cores = n_cores
  )
} else {
  # Windows - use parLapply
  log_message(paste0("Creating cluster with ", n_cores, " workers"))
  cl <- makeCluster(n_cores)
  log_message(paste0("Cluster created with ", length(cl), " workers"))
  
  # Export necessary objects to cluster
  clusterExport(cl, c(
    "process_imputation", "imputed_datasets", "ps_formula", 
    "n_trees", "cache_dir", "use_cache", "worker_assignments", "log_dir"
  ), envir = environment())
...
for (res in parallel_results) {
  ...
  # Log individual result summary in main log
  if (res$cached) {
    log_message(paste0("Imputation ", res$imputation, " (Worker ", res$worker_id, "): Loaded from cache"))
  } else {
    log_message(paste0("Imputation ", res$imputation, " (Worker ", res$worker_id, "): Fitted in ", res$time_mins, " minutes"))
  }
}

# Read and consolidate worker logs
log_message("=== Worker Log Summary ===")
for (worker in 1:n_cores) {
  worker_log_file <- file.path(log_dir, paste0("propensity_scores_worker_", worker, ".log"))
  if (file.exists(worker_log_file)) {
    worker_logs <- readLines(worker_log_file)
    log_message(paste0("Worker ", worker, " processed ", 
                      sum(grepl("started", worker_logs)), " imputations, ",
