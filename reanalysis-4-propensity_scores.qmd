...
process_imputation <- function(i) {
  ...
  if (use_cache && file.exists(cache_file)) {
    ...
    # Get the data for this imputation and ensure lapse is numeric
    imp_data <- data.frame(imputed_datasets[[i]])
    
    # Ensure lapse is numeric 0/1 (required by twang)
    if (!is.numeric(imp_data$lapse)) {
...
if (.Platform$OS.type == "unix") {
  ...
  clusterExport(cl, c(
    "cache_dir", "use_cache", "ps_formula", "imputed_datasets",
    "n_trees", "process_imputation"
  ), envir = environment())
  ...
  parallel_results <- parLapply(cl, 1:n_imputations, process_imputation)
