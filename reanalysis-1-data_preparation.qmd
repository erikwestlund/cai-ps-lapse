...
**Following the original analysis approach:**
...
2. **CCI/DCSI**: Rows with missing values will be excluded (complete case)
   - These are key confounders that cannot be reliably imputed
...
missing_summary <- df_analysis |>
  ...
  dplyr::mutate(
    ...
    handling = dplyr::case_when(
      variable %in% c("CCI", "DCSI") ~ "Exclude if missing",
