...
# Initialize logging
log_file <- init_log("data_preparation")
```

## Data Loading

```{r load-data}
log_message(paste0("Loading data from: ", reanalysis_data_file_path))
df_combined <- readr::read_csv(reanalysis_data_file_path)
log_message(paste0("Loaded ", nrow(df_combined), " observations"))
```
...
# Finalize logging
log_message(paste0("Data preparation completed: ", nrow(df_for_imputation), " observations ready for imputation"))
