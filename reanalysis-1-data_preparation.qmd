...
verification_results <- data.frame(
  Check = c("Excluded cohort with outcome VA (should be 0)",
            "Submitted cohort with intermediate VA (should be 0)"),
  Count = c(
    sum(df_analysis$cohort_id == "excluded" & !is.na(df_analysis$outcome_VA_logMAR)),
    sum(df_analysis$cohort_id == "submitted" & !is.na(df_analysis$intermediate_VA_logMAR))
  ),
  Status = c(
    ifelse(sum(df_analysis$cohort_id == "excluded" & !is.na(df_analysis$outcome_VA_logMAR)) == 0, "✓ Pass", "✗ Fail"),
    ifelse(sum(df_analysis$cohort_id == "submitted" & !is.na(df_analysis$intermediate_VA_logMAR)) == 0, "✓ Pass", "✗ Fail")
  )
)
...
variable_lists <- list(
  ...
  temporal_vars = c("cohort_id", "intermediate_VA_logMAR", "outcome_VA_logMAR", "baseline_VA_logMAR")
)
