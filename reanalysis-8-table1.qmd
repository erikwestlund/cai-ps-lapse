...
format_continuous_table <- function(original, submitted, excluded, imputed) {
  ...
  for (i in 1:length(vars)) {
    ...
    if (nrow(orig_row) > 0 && !is.na(orig_row$mean)) {
      table_df$Original[i] <- sprintf("%.4f (%.4f)", orig_row$mean, orig_row$sd)
    }
    
    # Submitted
    sub_row <- submitted[submitted$variable == v, ]
    if (nrow(sub_row) > 0 && !is.na(sub_row$mean)) {
      table_df$Submitted[i] <- sprintf("%.4f (%.4f)", sub_row$mean, sub_row$sd)
    }
    
    # Excluded
    exc_row <- excluded[excluded$variable == v, ]
    if (nrow(exc_row) > 0 && !is.na(exc_row$mean)) {
      table_df$Excluded[i] <- sprintf("%.4f (%.4f)", exc_row$mean, exc_row$sd)
    }
    
    # Imputed
    imp_row <- imputed[imputed$variable == v, ]
    if (nrow(imp_row) > 0 && !is.na(imp_row$mean)) {
      table_df$Imputed[i] <- sprintf("%.4f (%.4f)", imp_row$mean, imp_row$sd)
    }
  }
...
format_categorical_table <- function(original, submitted, excluded, imputed) {
  ...
  for (i in 1:length(vars)) {
    ...
    if (nrow(orig_row) > 0 && !is.na(orig_row$n)) {
      table_df$Original[i] <- sprintf("%.0f (%.4f%%)", orig_row$n, orig_row$pct)
    }
    
    # Submitted
    sub_row <- submitted[submitted$variable_std == v, ]
    if (nrow(sub_row) > 0 && !is.na(sub_row$n)) {
      table_df$Submitted[i] <- sprintf("%.0f (%.4f%%)", sub_row$n, sub_row$pct)
    }
    
    # Excluded
    exc_row <- excluded[excluded$variable_std == v, ]
    if (nrow(exc_row) > 0 && !is.na(exc_row$n)) {
      table_df$Excluded[i] <- sprintf("%.0f (%.4f%%)", exc_row$n, exc_row$pct)
    }
    
    # Imputed
    imp_row <- imputed[imputed$variable_std == v, ]
    if (nrow(imp_row) > 0 && !is.na(imp_row$n)) {
      table_df$Imputed[i] <- sprintf("%.1f (%.4f%%)", imp_row$n, imp_row$pct)
    }
  }
...
print(kable(sample_sizes, caption = "Sample Sizes"))
```
...
```{r stratified-complete-cases, results='asis'}
  ...
  if (is_continuous) {
    ...
    no_lapse_stats <- data |>
      ...
      mutate(
        ...
        stat_no_lapse = sprintf("%d (%.4f%%)", n, pct)
      ) |>
      rename(level = !!sym(var))
...
create_stratified_table <- function(data) {
  ...
  results$Total <- data.frame(
    Variable = "Total",
    Lapse = sprintf("%d (%.4f%%)", n_lapse, 100*n_lapse/n_total),
    NoLapse = sprintf("%d (%.4f%%)", n_no_lapse, 100*n_no_lapse/n_total),
  ...
  # Add overall total for treatments
  prp_overall <- data |>
    summarise(
      n = sum(PRP_num == 1, na.rm = TRUE),
      total = n(),
  ...
  prp_total <- bind_rows(prp_total, prp_overall)
  ...
  focal_total <- bind_rows(focal_total, focal_overall)
  ...
  vegf_total <- bind_rows(vegf_total, vegf_overall)
  ...
  other_inj_total <- bind_rows(other_inj_total, other_overall)
  ...
  glaucoma_total <- bind_rows(glaucoma_total, glaucoma_overall)
  ...
  cataract_total <- bind_rows(cataract_total, cataract_overall)
  ...
  retina_total <- bind_rows(retina_total, retina_overall)
...
format_stratified_table <- function(table_data, title) {
  ...
  # Display totals
  cat("**Total Patients**\n")
  print(knitr::kable(table_data$totals))
  
  cat("\n**Demographic Characteristics**\n")
  
  cat("\n*Sex*\n")
  print(knitr::kable(table_data$sex))
  
  cat("\n*Age (years)*\n")
  print(knitr::kable(table_data$age))
  
  cat("\n*Race/Ethnicity*\n")
  print(knitr::kable(table_data$race))
  
  cat("\n*Insurance*\n")
  print(knitr::kable(table_data$insurance))
  
  cat("\n**Ocular Characteristics**\n")
  
  cat("\n*Severity of DR*\n")
  print(knitr::kable(table_data$dr))
  
  cat("\n*DR Treatments (% with treatment)*\n")
  treatments_df <- data.frame(
    Treatment = c("PRP", "Focal laser", "Anti-VEGF", "Other injections"),
    Lapse = c(
      sprintf("%d (%.4f%%)", table_data$treatments$prp$n[1], table_data$treatments$prp$pct[1]),
      sprintf("%d (%.4f%%)", table_data$treatments$focal$n[1], table_data$treatments$focal$pct[1]),
      sprintf("%d (%.4f%%)", table_data$treatments$vegf$n[1], table_data$treatments$vegf$pct[1]),
      sprintf("%d (%.4f%%)", table_data$treatments$other$n[1], table_data$treatments$other$pct[1])
    ),
    NoLapse = c(
      sprintf("%d (%.4f%%)", table_data$treatments$prp$n[2], table_data$treatments$prp$pct[2]),
      sprintf("%d (%.4f%%)", table_data$treatments$focal$n[2], table_data$treatments$focal$pct[2]),
      sprintf("%d (%.4f%%)", table_data$treatments$vegf$n[2], table_data$treatments$vegf$pct[2]),
      sprintf("%d (%.4f%%)", table_data$treatments$other$n[2], table_data$treatments$other$pct[2])
    ),
    Total = c(
      sprintf("%d (%.4f%%)", table_data$treatments$prp$n[3], table_data$treatments$prp$pct[3]),
      sprintf("%d (%.4f%%)", table_data$treatments$focal$n[3], table_data$treatments$focal$pct[3]),
      sprintf("%d (%.4f%%)", table_data$treatments$vegf$n[3], table_data$treatments$vegf$pct[3]),
      sprintf("%d (%.4f%%)", table_data$treatments$other$n[3], table_data$treatments$other$pct[3])
    )
  )
  print(knitr::kable(treatments_df, col.names = c("Treatment", "Lapse in care", "No lapse in care", "Total")))
  
  cat("\n*Ophthalmologic Comorbidities*\n")
  comorb_df <- data.frame(
    Condition = c("Glaucoma", "Prior cataract surgery", "Other retinal disease"),
    Lapse = c(
      sprintf("%d (%.4f%%)", table_data$comorbidities$glaucoma$n[1], table_data$comorbidities$glaucoma$pct[1]),
      sprintf("%d (%.4f%%)", table_data$comorbidities$cataract$n[1], table_data$comorbidities$cataract$pct[1]),
      sprintf("%d (%.4f%%)", table_data$comorbidities$retina$n[1], table_data$comorbidities$retina$pct[1])
    ),
    NoLapse = c(
      sprintf("%d (%.4f%%)", table_data$comorbidities$glaucoma$n[2], table_data$comorbidities$glaucoma$pct[2]),
      sprintf("%d (%.4f%%)", table_data$comorbidities$cataract$n[2], table_data$comorbidities$cataract$pct[2]),
      sprintf("%d (%.4f%%)", table_data$comorbidities$retina$n[2], table_data$comorbidities$retina$pct[2])
    ),
    Total = c(
      sprintf("%d (%.4f%%)", table_data$comorbidities$glaucoma$n[3], table_data$comorbidities$glaucoma$pct[3]),
      sprintf("%d (%.4f%%)", table_data$comorbidities$cataract$n[3], table_data$comorbidities$cataract$pct[3]),
      sprintf("%d (%.4f%%)", table_data$comorbidities$retina$n[3], table_data$comorbidities$retina$pct[3])
    )
  )
  print(knitr::kable(comorb_df, col.names = c("Condition", "Lapse in care", "No lapse in care", "Total")))
  ...
  print(knitr::kable(clinical_df, col.names = c("Variable", "Lapse in care", "No lapse in care", "Total")))
}

format_stratified_table(complete_table_data, "Table 1: Baseline Characteristics by Lapse Status (Complete Cases)")

# Export complete cases table to CSV
export_stratified_table <- function(table_data, filename) {
  # Initialize empty dataframe for export
  export_df <- data.frame(
    variable = character(),
    lapse_value = numeric(),
    lapse_prop = numeric(),
    lapse_sd = numeric(),
    no_lapse_value = numeric(),
    no_lapse_prop = numeric(),
    no_lapse_sd = numeric(),
    total_value = numeric(),
    total_prop = numeric(),
    total_sd = numeric(),
  ...
  # Helper function to parse "mean ± sd" format
  parse_mean_sd <- function(x) {
    if (is.na(x) || x == "") return(list(mean = NA, sd = NA))
    parts <- strsplit(x, " ± ")[[1]]
    return(list(mean = as.numeric(parts[1]), sd = as.numeric(parts[2])))
  }
  
  # Helper function to parse "n (pct%)" format
  parse_n_pct <- function(x) {
    if (is.na(x) || x == "") return(list(n = NA, pct = NA))
    n <- as.numeric(gsub(" .*", "", x))
    pct <- as.numeric(gsub(".*\\((.*)%.*", "\\1", x))
    return(list(n = n, pct = pct))
  }
  
  # Add total row
  lapse_total <- parse_n_pct(table_data$totals$Lapse)
  no_lapse_total <- parse_n_pct(table_data$totals$NoLapse)
  total_total <- parse_n_pct(table_data$totals$Total)
  
  export_df <- rbind(export_df, data.frame(
    variable = "Total",
    lapse_value = lapse_total$n,
    lapse_prop = round(lapse_total$pct, 4),
    lapse_sd = NA,
    no_lapse_value = no_lapse_total$n,
    no_lapse_prop = round(no_lapse_total$pct, 4),
    no_lapse_sd = NA,
    total_value = total_total$n,
    total_prop = 100,
    total_sd = NA
  ))
  ...
  # Sex
  if (!is.null(table_data$sex)) {
    for (i in 1:nrow(table_data$sex)) {
      level <- table_data$sex$level[i]
      lapse_val <- parse_n_pct(table_data$sex$stat_lapse[i])
      no_lapse_val <- parse_n_pct(table_data$sex$stat_no_lapse[i])
      total_val <- parse_n_pct(table_data$sex$stat_total[i])
      
      export_df <- rbind(export_df, data.frame(
        variable = paste0("Sex_", level),
        lapse_value = lapse_val$n,
        lapse_prop = round(lapse_val$pct, 4),
        lapse_sd = NA,
        no_lapse_value = no_lapse_val$n,
        no_lapse_prop = round(no_lapse_val$pct, 4),
        no_lapse_sd = NA,
        total_value = total_val$n,
        total_prop = round(total_val$pct, 4),
        total_sd = NA
      ))
    }
  }
  ...
  # Age groups
  if (!is.null(table_data$age)) {
    for (i in 1:nrow(table_data$age)) {
      level <- table_data$age$level[i]
      lapse_val <- parse_n_pct(table_data$age$stat_lapse[i])
      no_lapse_val <- parse_n_pct(table_data$age$stat_no_lapse[i])
      total_val <- parse_n_pct(table_data$age$stat_total[i])
      
      export_df <- rbind(export_df, data.frame(
        variable = paste0("Age_", level),
        lapse_value = lapse_val$n,
        lapse_prop = round(lapse_val$pct, 4),
        lapse_sd = NA,
        no_lapse_value = no_lapse_val$n,
  ...
  # Race/Ethnicity
  if (!is.null(table_data$race)) {
    for (i in 1:nrow(table_data$race)) {
      level <- table_data$race$level[i]
      lapse_val <- parse_n_pct(table_data$race$stat_lapse[i])
      no_lapse_val <- parse_n_pct(table_data$race$stat_no_lapse[i])
      total_val <- parse_n_pct(table_data$race$stat_total[i])
      
      export_df <- rbind(export_df, data.frame(
        variable = paste0("Race_", level),
        lapse_value = lapse_val$n,
        lapse_prop = round(lapse_val$pct, 4),
        lapse_sd = NA,
        no_lapse_value = no_lapse_val$n,
  ...
  # Insurance
  if (!is.null(table_data$insurance)) {
    for (i in 1:nrow(table_data$insurance)) {
      level <- table_data$insurance$level[i]
      lapse_val <- parse_n_pct(table_data$insurance$stat_lapse[i])
      no_lapse_val <- parse_n_pct(table_data$insurance$stat_no_lapse[i])
      total_val <- parse_n_pct(table_data$insurance$stat_total[i])
      
      export_df <- rbind(export_df, data.frame(
        variable = paste0("Insurance_", level),
        lapse_value = lapse_val$n,
        lapse_prop = round(lapse_val$pct, 4),
        lapse_sd = NA,
        no_lapse_value = no_lapse_val$n,
  ...
  # DR Severity
  if (!is.null(table_data$dr)) {
    for (i in 1:nrow(table_data$dr)) {
      level <- table_data$dr$level[i]
      lapse_val <- parse_n_pct(table_data$dr$stat_lapse[i])
      no_lapse_val <- parse_n_pct(table_data$dr$stat_no_lapse[i])
      total_val <- parse_n_pct(table_data$dr$stat_total[i])
      
      export_df <- rbind(export_df, data.frame(
        variable = paste0("DR_", level),
        lapse_value = lapse_val$n,
        lapse_prop = lapse_val$pct,
        lapse_sd = NA,
        no_lapse_value = no_lapse_val$n,
        no_lapse_prop = no_lapse_val$pct,
        no_lapse_sd = NA,
        total_value = total_val$n,
        total_prop = total_val$pct,
        total_sd = NA
      ))
    }
  }
  ...
  # Treatments
  treatment_names <- c("PRP", "Focal_laser", "Anti_VEGF", "Other_injections")
  treatment_data <- list(table_data$treatments$prp, table_data$treatments$focal,
                        table_data$treatments$vegf, table_data$treatments$other)
  
  for (j in 1:length(treatment_names)) {
    if (!is.null(treatment_data[[j]])) {
      tx_data <- treatment_data[[j]]
      export_df <- rbind(export_df, data.frame(
        variable = treatment_names[j],
        lapse_value = tx_data$n[tx_data$lapse_group == "Lapse in care"],
        lapse_prop = round(tx_data$pct[tx_data$lapse_group == "Lapse in care"], 4),
        lapse_sd = NA,
        no_lapse_value = tx_data$n[tx_data$lapse_group == "No lapse in care"],
        no_lapse_prop = round(tx_data$pct[tx_data$lapse_group == "No lapse in care"], 4),
        no_lapse_sd = NA,
        total_value = tx_data$n[tx_data$lapse_group == "Total"],
        total_prop = round(tx_data$pct[tx_data$lapse_group == "Total"], 4),
        total_sd = NA
      ))
    }
  }
  ...
  # Comorbidities
  comorb_names <- c("Glaucoma", "Prior_cataract_surgery", "Other_retinal_disease")
  comorb_data <- list(table_data$comorbidities$glaucoma, table_data$comorbidities$cataract,
                     table_data$comorbidities$retina)
  
  for (j in 1:length(comorb_names)) {
    if (!is.null(comorb_data[[j]])) {
      com_data <- comorb_data[[j]]
      export_df <- rbind(export_df, data.frame(
        variable = comorb_names[j],
        lapse_value = com_data$n[com_data$lapse_group == "Lapse in care"],
        lapse_prop = round(com_data$pct[com_data$lapse_group == "Lapse in care"], 4),
        lapse_sd = NA,
        no_lapse_value = com_data$n[com_data$lapse_group == "No lapse in care"],
        no_lapse_prop = round(com_data$pct[com_data$lapse_group == "No lapse in care"], 4),
        no_lapse_sd = NA,
        total_value = com_data$n[com_data$lapse_group == "Total"],
        total_prop = round(com_data$pct[com_data$lapse_group == "Total"], 4),
        total_sd = NA
      ))
    }
  }
  ...
  # Continuous variables
  cont_vars <- list(
    list(name = "Baseline_VA_logMAR", data = table_data$continuous$va),
    list(name = "DCSI", data = table_data$continuous$dcsi),
    list(name = "CCI", data = table_data$continuous$cci)
  )
  
  for (cv in cont_vars) {
    if (!is.null(cv$data)) {
      lapse_stats <- parse_mean_sd(cv$data$stat[cv$data$lapse_group == "Lapse in care"])
      no_lapse_stats <- parse_mean_sd(cv$data$stat[cv$data$lapse_group == "No lapse in care"])
      total_stats <- parse_mean_sd(cv$data$stat[cv$data$lapse_group == "Total"])
      
      export_df <- rbind(export_df, data.frame(
        variable = cv$name,
        lapse_value = ifelse(is.na(lapse_stats$mean), NA, round(lapse_stats$mean, 4)),
        lapse_prop = NA,
        lapse_sd = ifelse(is.na(lapse_stats$sd), NA, round(lapse_stats$sd, 4)),
        no_lapse_value = ifelse(is.na(no_lapse_stats$mean), NA, round(no_lapse_stats$mean, 4)),
        no_lapse_prop = NA,
        no_lapse_sd = ifelse(is.na(no_lapse_stats$sd), NA, round(no_lapse_stats$sd, 4)),
        total_value = ifelse(is.na(total_stats$mean), NA, round(total_stats$mean, 4)),
        total_prop = NA,
        total_sd = ifelse(is.na(total_stats$sd), NA, round(total_stats$sd, 4))
      ))
    }
  }
...
# Export the complete cases table
complete_export <- export_stratified_table(complete_table_data, 
                                           file.path(reanalysis_data_dir, "table1_complete_cases.csv"))
```

## Stratified Table 2: Imputed Data (Averaged)

```{r stratified-imputed, results='asis'}
...
create_imputed_stratified_table <- function(imputed_list) {
  ...
  avg_totals <- data.frame(
    ...
    })), 4), " (", round(mean(sapply(all_results, function(x) {
...
# Export the imputed table
imputed_export <- export_stratified_table(imputed_table_data, 
                                          file.path(reanalysis_data_dir, "table1_imputed.csv"))
```
