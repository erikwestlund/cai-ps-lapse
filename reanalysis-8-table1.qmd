...
export_stratified_table <- function(table_data, filename) {
  ...
  # Add total row
  n_lapse <- as.numeric(gsub(" .*", "", table_data$totals$Lapse))
  n_no_lapse <- as.numeric(gsub(" .*", "", table_data$totals$NoLapse))
  n_total <- as.numeric(gsub(" .*", "", table_data$totals$Total))
  
  export_df <- rbind(export_df, data.frame(
    variable = "Total",
    lapse_value = n_lapse,
    lapse_prop = as.numeric(gsub(".*\\((.*)%.*", "\\1", table_data$totals$Lapse)),
    lapse_sd = NA,
    no_lapse_value = n_no_lapse,
    no_lapse_prop = as.numeric(gsub(".*\\((.*)%.*", "\\1", table_data$totals$NoLapse)),
    no_lapse_sd = NA,
    total_value = n_total,
    total_prop = 100,
    total_sd = NA
  ...
  # Process categorical variables from table_data
  # This is a simplified version - you'll need to expand this based on your exact structure
  
  # Add treatments
  for (i in 1:3) {  # Lapse, No Lapse, Total
    if (i <= nrow(table_data$treatments$prp)) {
      # Extract values for each treatment
      # This needs more sophisticated parsing based on actual data structure
    }
  }
  
  # Add continuous variables
  for (var_name in c("baseline_VA_logMAR", "DCSI", "CCI")) {
    ...
    if (var_name == "baseline_VA_logMAR") {
      stats <- table_data$continuous$va$stat
    } else if (var_name == "DCSI") {
      stats <- table_data$continuous$dcsi$stat
    } else {
      stats <- table_data$continuous$cci$stat
    }
    ...
    lapse_stats <- parse_mean_sd(stats[1])
    no_lapse_stats <- parse_mean_sd(stats[2])
    total_stats <- parse_mean_sd(stats[3])
