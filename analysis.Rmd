...
run_matching_methods <- function(data, formula, methods, rerun_model = FALSE, rerun_diagnostics = TRUE, output_dir = "ps_results") {
  ...
  for (params in methods) {
    ...
    if (rerun_diagnostics || !file.exists(diagnostics_filename)) {
      ...
      saveRDS(list(balance_plot = balance_plot, common_support_plot = common_support_plot, summary = balance_summary), file = diagnostics_filename)
    } else {
      print("Loading cached diagnostics...")
      diagnostics <- readRDS(diagnostics_filename)
      balance_plot <- diagnostics$balance_plot
      common_support_plot <- diagnostics$common_support_plot
