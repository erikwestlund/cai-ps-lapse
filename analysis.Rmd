...
run_matching_methods <- function(data, formula, methods, rerun_all = FALSE, output_dir = "ps_results") {
  ...
  for (params in methods) {
    ...
    if (file.exists(filename) && !params$rerun && !rerun_all) {
    ...
    if (params$method == "twang") {
      result <- tryCatch({
        ps(formula = formula, data = data.frame(data), 
           ...
           stop.method = c("es.mean", "ks.max"))  # Stopping criteria
      }, error = function(e) {
        message("Error in TWANG GBM: ", e)
        return(NULL)
      })
      
      if (!is.null(result)) {
        diagnostic_plot <- tryCatch({
          love.plot(result, stat = "mean.diffs", var.order = "es.mean", stars="std")
        }, error = function(e) {
          message("Error generating TWANG balance plot: ", e)
          NULL
        })
        
        equipoise_plot <- tryCatch({
          bal.plot(result, var.name = "prop.score.es.mean", which="both")
        }, error = function(e) {
          message("Error generating TWANG equipoise plot: ", e)
          NULL
        })
        ...
        balance_summary <- tryCatch({
          ...
          message("Error generating TWANG balance summary: ", e)
          NULL
        })
      }
      
    } else if(length(params$use) > 0 && params$use == "weightit") {
       result <- tryCatch({
      ...
      diagnostic_plot <- tryCatch({
        ...
        message("Error generating diagnostic plot for ", params$name, ": ", e)
        NULL
      })
      
      if(! params$method %in% c("nearest_mahalanobis", "cem", "cbps", "entropy")) {
        equipoise_plot <- tryCatch({
          bal.plot(result, var.name = "distance", which = "both")
        }, error = function(e) {
          message("Error generating equipoise plot for ", params$name, ": ", e)
          NULL
        })
      } else {
        equipoise_plot <- NULL
...
methods <- list(
  list(name = "nearest_glm", method = "nearest", distance = "glm", replace = TRUE, estimand="ATT", rerun = FALSE),
  list(name = "nearest_gam", method = "nearest", distance = "gam", replace = TRUE, estimand="ATT", rerun = FALSE),
  list(name = "nearest_gbm", method = "nearest", distance = "gbm", replace = TRUE, estimand="ATT", rerun = FALSE),
  list(name = "nearest_lasso", method = "nearest", distance = "lasso", replace = TRUE, estimand="ATT", rerun = FALSE),
  list(name = "nearest_rpart", method = "nearest", distance = "rpart", replace = TRUE, estimand="ATT", rerun = FALSE),
  list(name = "subclass_glm", method = "subclass", distance = "glm", subclasses = 5, estimand="ATT", rerun = FALSE),
  list(name = "cem", method = "cem", estimand="ATT", rerun = FALSE),
  list(name = "nearest_mahalanobis", method = "nearest", distance = "mahalanobis", replace = TRUE, estimand="ATT", rerun = FALSE),
  list(name = "cbps", use="weightit", method="cbps", estimand="ATT", rerun=FALSE),
  list(name = "entropy", use="weightit", method="ebal", estimand="ATT", rerun=FALSE),
  list(name = "bart", use="weightit", method="bart", estimand="ATT", rerun=FALSE),
  list(name = "twang_gbm", use="twanng", method = "twang", estimand="ATT", rerun = FALSE)
)
...
```{r diagnostic_plots}
for (method_name in method_names) {
  # Get diagnostic and equipoise plots for the method
  method_results <- results[[method_name]]
  diagnostic_plot <- method_results$diagnostic_plot
  equipoise_plot <- method_results$equipoise_plot
  
  # Output section header for diagnostic plot
  ...
  # Output section header for equipoise plot (if exists)
  if (!is.null(equipoise_plot)) {
    cat("### Equipoise Plot for", method_name, "\n\n")
    print(equipoise_plot)
    cat("\n\n")
  }
}
```
