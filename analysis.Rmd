...
run_matching_methods <- function(data, formula, methods, output_dir = "ps_results") {
  ...
  for (params in methods) {
    ...
    if (params$rerun_diagnostics || !file.exists(diagnostics_filename)) {
      ...
      saveRDS(list(love_plot = love_plot, balance_plot = balance_plot, summary = balance_summary), file = diagnostics_filename)
    } else {
...
run_logistic_models <- function(design, formulas, method_name) {
  ...
  for (i in seq_along(formulas)) {
    if (method_name %in% c("cbps", "entropy", "bart")) {
      ...
      package <- "twang"
      approach <- "ipw"
    } else if (method_name == "subclass") {
      package <- "MatchIt"
      approach <- "subclassification"
    } else {
      package <- "MatchIt"
      approach <- "match w/ replacement"
    }

    if (method_name == "subclass") {
      # Extract subclass weights
      subclass_weights <- weights(design, type = "sampling")

      # Run weighted survey logistic regression
      model <- svyglm(formula = formulas[[i]], design = design, family = quasibinomial(), contrasts = NULL)
    } else {
      # Standard survey logistic regression
      model <- svyglm(formula = formulas[[i]], design = design, family = quasibinomial(), contrasts = NULL)
    }
...
for (method_name in method_names) {
  ...
  if(method_name %in% c("cbps", "entropy", "energy", "bart")) {
    ...
    matched_data <- if(method_name == "subclass_glm") {
      MatchIt::match_data(method_result$result, subclass = 5)
    } else {
