...
message("  - stop.method: es.mean, ks.max")
...
for (i in 1:n_imputations) {
  ...
  # Extract weights for ATT (using es.mean as primary stop method)
  ...
  twang_results[[i]] <- list(
    model = twang_model,
    weights = weights,
    ps_scores = twang_model$ps[, "es.mean"],
    n_trees_used = twang_model$desc$es.mean$n.trees,
    balance_es_mean = twang_model$balance$es.mean,
    balance_ks_max = twang_model$balance$ks.max
  )
  
  message("  - Trees used (es.mean): ", twang_model$desc$es.mean$n.trees)
  message("  - Trees used (ks.max): ", twang_model$desc$ks.max$n.trees)
}
...
for (i in 1:n_imputations) {
  # Get balance table from twang
  bal_es <- twang_results[[i]]$balance_es_mean
  bal_ks <- twang_results[[i]]$balance_ks_max
  
  # Summary statistics for es.mean method
  balance_summaries[[i]] <- data.frame(
    Imputation = i,
    Mean_ES = mean(abs(bal_es$std.eff.sz), na.rm = TRUE),
    Max_ES = max(abs(bal_es$std.eff.sz), na.rm = TRUE),
    N_Imbalanced = sum(abs(bal_es$std.eff.sz) > 0.1, na.rm = TRUE),
    Mean_KS = mean(bal_ks$ks, na.rm = TRUE),
    Max_KS = max(bal_ks$ks, na.rm = TRUE)
  )
}
...
message("Stop methods: es.mean (effect size), ks.max (Kolmogorov-Smirnov)")
