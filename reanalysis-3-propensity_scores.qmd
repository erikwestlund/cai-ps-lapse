...
for (i in imps_to_show) {
  ...
  # Extract propensity scores - twang stores them in the ps slot
  ps_scores <- tryCatch({
    if (!is.null(twang_model$ps)) {
      # ps can be a matrix (multiple stop methods) or vector (single stop method)
      if (is.matrix(twang_model$ps) || is.data.frame(twang_model$ps)) {
        twang_model$ps[, 1]  # Get first column
      } else {
        twang_model$ps  # Already a vector
      }
    } else {
      # Shouldn't happen but fallback to weights
      weights
    }
  }, error = function(e) {
    weights  # Use weights as fallback
  })
  ...
  # Control group
  hist(ps_scores[!treated], 
       main = paste("Control - Imputation", i),
  ...
  # Treated group  
  hist(ps_scores[treated],
       main = paste("Treated - Imputation", i),
       xlab = "Propensity Score", 
       col = "lightcoral",
       breaks = 20)
