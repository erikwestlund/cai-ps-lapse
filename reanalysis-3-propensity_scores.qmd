...
for (i in 1:n_imputations) {
  ...
  # Get balance table - use summary or bal.table depending on what works
  balance_table <- tryCatch({
    # Try to get balance table
    if (!is.null(twang_model$balance.es.mean)) {
      twang_model$balance.es.mean
    } else if (!is.null(twang_model$balance)) {
      twang_model$balance
    } else {
      # Extract from summary
      summary(twang_model)$es.mean.ATT
    }
  }, error = function(e) {
    # If all else fails, create a simple balance assessment
    NULL
  })
...
for (i in 1:n_imputations) {
  # Use summary to get balance statistics
  twang_summary <- summary(twang_results[[i]]$model)
  
  # The summary contains balance tables for es.mean.ATT
  if (!is.null(twang_summary$es.mean.ATT)) {
    bal_table <- twang_summary$es.mean.ATT
    
    # Extract effect sizes - look for appropriate columns
    es_values <- if ("tx.mn" %in% names(bal_table) && "ct.mn" %in% names(bal_table)) {
      # Calculate standardized differences manually if needed
      tx_means <- bal_table$tx.mn
      ct_means <- bal_table$ct.mn
      tx_sd <- if("tx.sd" %in% names(bal_table)) bal_table$tx.sd else rep(1, length(tx_means))
      ct_sd <- if("ct.sd" %in% names(bal_table)) bal_table$ct.sd else rep(1, length(ct_means))
      pooled_sd <- sqrt((tx_sd^2 + ct_sd^2) / 2)
      abs((tx_means - ct_means) / pooled_sd)
    } else if ("std.eff.sz" %in% names(bal_table)) {
      abs(bal_table$std.eff.sz)
    } else if ("std.es" %in% names(bal_table)) {
      abs(bal_table$std.es)
    } else {
      # Try to find any column with "std" or "es"
      std_cols <- grep("std|es", names(bal_table), value = TRUE)
      if (length(std_cols) > 0) {
        abs(bal_table[[std_cols[1]]])
      } else {
    ...
    # Fallback if summary doesn't have expected structure
    balance_summaries[[i]] <- data.frame(
      Imputation = i,
      Mean_ES = NA,
      Max_ES = NA,
    ...
    message("Warning: Could not extract balance statistics for imputation ", i)
  }
}
...
for (i in imps_to_show) {
  ...
  # Plot 1: Balance by iteration (shows convergence)
  plot(twang_model, plots = 1)
  
  # Plot 2: Standardized effect size by iteration
  plot(twang_model, plots = 2)
  
  # Plot 3: Balance table plot (if available)
  plot(twang_model, plots = 3)
  
  # Plot 4: Boxplot of propensity scores by treatment
  plot(twang_model, plots = 4)
  
  # Additional balance summary plot using twang's summary
  summary(twang_model)
}
```
