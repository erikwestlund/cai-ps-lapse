...
message("Calculating propensity scores using Twang GBM...")
message("Parameters:")
...
message("Twang propensity score calculation complete")
```
...
for (i in 1:n_imputations) {
  ...
  if (i <= max_imputations_for_diagnostics) {
    message("Balance summary for imputation ", i, ":")
...
message("Weight Summary Across All Imputations:")
...
message("Average Weights Across Imputations:")
kable(avg_weights,
...
if (avg_weights$Max_Weight_Overall > 10) {
  message("⚠ Warning: Maximum weight > 10, indicating potential instability")
} else if (avg_weights$Max_Weight_Overall > 5) {
  message("Note: Maximum weight > 5, monitor for stability")
} else {
  message("✓ Weights appear stable (max < 5)")
}

message("Note: Balance statistics are shown in the summary output above.")
...
message("Generating twang diagnostic plots for ", n_to_show, " imputations...")

# For each selected imputation, show twang-specific diagnostics
for (i in imps_to_show) {
  message("Diagnostics for Imputation ", i, ":")
...
for (i in imps_to_show) {
  message("Propensity score distributions for imputation ", i)
...
message("Twang propensity score results saved to: ", 
...
```{r summary}
# Final summary
message("=== TWANG PROPENSITY SCORE ANALYSIS SUMMARY ===")
...
if (avg_weights$Max_Weight_Overall > 10) {
  ...
  message("Note: Maximum weight > 5, monitor for stability in outcome analysis")
} else {
  message("✓ Weights appear stable (max < 5)")
}

message("Balance Assessment:")
