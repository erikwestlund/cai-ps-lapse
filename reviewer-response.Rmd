...
extract_predictions <- function(model, data, interaction_var, fixed_vars = NULL) {
  ...
  # Get predictions (handle survey model output)
  preds <- predict(model, newdata = pred_data, type = "response")
  
  # Extract values depending on the structure returned
  if(is.matrix(preds) || is.data.frame(preds)) {
    pred_data$predicted <- as.numeric(preds[,1])
  } else {
  ...
  # Get standard errors
  se_preds <- tryCatch({
    predict(model, newdata = pred_data, type = "response", se.fit = TRUE)$se.fit
  }, error = function(e) {
    # If se.fit doesn't work, try to get SE another way
    se_obj <- predict(model, newdata = pred_data, type = "link", se.fit = TRUE)
    # Convert SE from link scale to response scale using delta method
    link_pred <- se_obj$fit
    link_se <- se_obj$se.fit
    # For logit link: SE(p) ≈ SE(logit) * p * (1-p)
    p <- plogis(link_pred)
    return(link_se * p * (1 - p))
  })
  
  if(is.matrix(se_preds) || is.data.frame(se_preds)) {
    pred_data$se <- as.numeric(se_preds[,1])
  } else {
    pred_data$se <- as.numeric(se_preds)
  }
