...
extract_predictions <- function(model, data, interaction_var, fixed_vars = NULL) {
  ...
  # Get predictions on link scale first
  link_pred <- predict(model, newdata = pred_data, type = "link", se.fit = TRUE)
  
  # Convert to response scale
  pred_data$predicted <- plogis(as.numeric(link_pred$fit))
  
  # Calculate SEs on response scale using delta method
  link_se <- as.numeric(link_pred$se.fit)
  pred_data$se <- link_se * pred_data$predicted * (1 - pred_data$predicted)
