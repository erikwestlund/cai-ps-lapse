...
# Forest plot - Odds ratios only
p_forest <- ggplot(plot_data, aes(x = DR_Severity, y = Odds_Ratio)) +
...
## Average Marginal Effects Visualization

```{r marginal_effects_viz, fig.height=5, fig.width=8}
# Calculate average marginal effects using margins or emmeans
# This shows the actual effect of lapsing within each group
ame_plot <- ggplot(plot_data, aes(x = DR_Severity, y = Log_Odds)) +
  ...
  labs(title = "Average Marginal Effect of Lapsing by DR Severity",
       subtitle = "Effect on log odds of vision impairment with 95% CIs",
       x = "DR Severity",
       y = "Average Marginal Effect (Log Odds)") +
  theme_minimal() +
  theme(plot.title = element_text(size = 14, face = "bold"),
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 12)) +
  # Add annotations for sample sizes
  annotate("text", x = 1, y = min(plot_data$CI_Lower) - 0.1, 
           label = "n=16,130", size = 3.5, color = "gray50") +
  annotate("text", x = 2, y = min(plot_data$CI_Lower) - 0.1, 
           label = "n=5,695", size = 3.5, color = "gray50") +
  annotate("text", x = 3, y = min(plot_data$CI_Lower) - 0.1, 
