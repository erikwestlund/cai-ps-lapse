...
for(dr_level in c("No_DR", "NPDR", "PDR")) {
  cat(paste("Calculating AME for", dr_level, "...\n"))
  ...
  if("twang_att_w" %in% names(subset_data)) {
    ...
    ame_value <- weighted.mean(individual_effects, w = subset_weights, na.rm = TRUE)
    # Weighted standard error
    weighted_var <- sum(subset_weights * (individual_effects - ame_value)^2) / sum(subset_weights)
    se_value <- sqrt(weighted_var / n_subset)
  } else {
    # Simple mean if no weights
    ame_value <- mean(individual_effects, na.rm = TRUE)
    se_value <- sd(individual_effects, na.rm = TRUE) / sqrt(n_subset)
  }
