...
calculate_marginal_effects <- function(model, data, subgroup_var, subgroup_levels) {
  ...
  for(level in subgroup_levels) {
    # Create prediction data for lapse = 0 and lapse = 1
    pred_data_0 <- pred_data_1 <- data[1:100, ]  # Use subset for speed
    pred_data_0$ever_lapse_binary <- 0
    pred_data_1$ever_lapse_binary <- 1
    
    # Set the subgroup to current level
    if(subgroup_var == "person_dr") {
      pred_data_0$person_dr <- level
      pred_data_1$person_dr <- level
    ...
    # Calculate marginal effect (difference in predictions)
    if(inherits(pred_0, "svystat")) {
      me <- mean(as.numeric(pred_1) - as.numeric(pred_0))
      # Bootstrap CI
      me_se <- sd(as.numeric(pred_1) - as.numeric(pred_0)) / sqrt(length(pred_1))
    } else {
      me <- mean(pred_1 - pred_0)
      me_se <- sd(pred_1 - pred_0) / sqrt(length(pred_1))
    }
