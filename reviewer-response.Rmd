...
## Average Marginal Effects Visualization (Probability Scale)

```{r marginal_effects_viz, fig.height=5, fig.width=8}
# Convert log odds AMEs to probability differences
# Using the delta method approximation or simulation
# For interpretability, we'll calculate the average marginal effect on the probability scale

# Get baseline predicted probabilities (at means of covariates)
# For a rough approximation, use the intercept to get baseline risk
intercept <- coef(model_dr_interaction)["(Intercept)"]
baseline_prob <- exp(intercept) / (1 + exp(intercept))

# Convert log odds effects to approximate probability differences
# Using the derivative of the logistic function at the baseline probability
# d(prob)/d(log_odds) = prob * (1 - prob)
prob_scale_factor <- baseline_prob * (1 - baseline_prob)

# Create probability scale effects
plot_data_prob <- plot_data |>
  mutate(
    AME_Prob = Log_Odds * prob_scale_factor,
    CI_Lower_Prob = CI_Lower * prob_scale_factor,
    CI_Upper_Prob = CI_Upper * prob_scale_factor,
    # Convert to percentage points for easier interpretation
    AME_Pct = AME_Prob * 100,
    CI_Lower_Pct = CI_Lower_Prob * 100,
    CI_Upper_Pct = CI_Upper_Prob * 100
  )

# Plot average marginal effects on probability scale
ame_plot <- ggplot(plot_data_prob, aes(x = DR_Severity, y = AME_Pct)) +
  ...
  labs(title = "Average Marginal Effect of Lapsing by DR Severity",
       subtitle = "Effect on probability of vision impairment (percentage points) with 95% CIs",
       x = "DR Severity",
  ...
  annotate("text", x = 1, y = min(plot_data_prob$CI_Lower_Pct) - 1, 
           label = "n=16,130", size = 3.5, color = "gray50") +
  annotate("text", x = 2, y = min(plot_data_prob$CI_Lower_Pct) - 1, 
           label = "n=5,695", size = 3.5, color = "gray50") +
  annotate("text", x = 3, y = min(plot_data_prob$CI_Lower_Pct) - 1, 
           label = "n=3,187", size = 3.5, color = "gray50")

print(ame_plot)

# Also display the probability scale effects in a table
prob_effects_table <- plot_data_prob |>
  select(DR_Severity, AME_Pct, CI_Lower_Pct, CI_Upper_Pct) |>
  mutate(
    AME_Pct = round(AME_Pct, 2),
    CI_Lower_Pct = round(CI_Lower_Pct, 2),
    CI_Upper_Pct = round(CI_Upper_Pct, 2),
    Effect = paste0(AME_Pct, "% (", CI_Lower_Pct, "%, ", CI_Upper_Pct, "%)")
  ) |>
  select(DR_Severity, Effect)

knitr::kable(prob_effects_table,
             caption = "Average Marginal Effect of Lapsing (Percentage Point Change in Probability)",
             col.names = c("DR Severity", "Effect (95% CI)"),
             row.names = FALSE)
```
