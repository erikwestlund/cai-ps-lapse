...
# For each DR severity group
# No DR
design_no_dr <- subset(survey_design_weighted, dr_severity == "No_DR")
data_no_dr <- subset(m_data, dr_severity == "No_DR")
n_no_dr <- nrow(data_no_dr)
mfx_no_dr <- summary(margins(model_dr_interaction, 
                             design = design_no_dr, 
                             data = data_no_dr, 
                             variables = "ever_lapse_binary"))

# NPDR
design_npdr <- subset(survey_design_weighted, dr_severity == "NPDR")
data_npdr <- subset(m_data, dr_severity == "NPDR")
n_npdr <- nrow(data_npdr)
mfx_npdr <- summary(margins(model_dr_interaction, 
                            design = design_npdr, 
                            data = data_npdr, 
                            variables = "ever_lapse_binary"))

# PDR
design_pdr <- subset(survey_design_weighted, dr_severity == "PDR")
data_pdr <- subset(m_data, dr_severity == "PDR")
n_pdr <- nrow(data_pdr)
mfx_pdr <- summary(margins(model_dr_interaction, 
                           design = design_pdr, 
                           data = data_pdr, 
                           variables = "ever_lapse_binary"))
...
ame_results <- data.frame(
  DR_Severity = c("Overall", "No_DR", "NPDR", "PDR"),
  N = c(nrow(m_data), n_no_dr, n_npdr, n_pdr),
  AME = c(mfx_overall$AME, mfx_no_dr$AME, mfx_npdr$AME, mfx_pdr$AME),
  SE = c(mfx_overall$SE, mfx_no_dr$SE, mfx_npdr$SE, mfx_pdr$SE),
  CI_Lower = c(mfx_overall$lower, mfx_no_dr$lower, mfx_npdr$lower, mfx_pdr$lower),
  CI_Upper = c(mfx_overall$upper, mfx_no_dr$upper, mfx_npdr$upper, mfx_pdr$upper),
  P_Value = c(mfx_overall$p, mfx_no_dr$p, mfx_npdr$p, mfx_pdr$p)
)
...
cat("\nThis approach matches the methodology used in twang-analysis.Rmd.\n")
```
...
if(exists("ame_results") && nrow(ame_results) > 0) {
  ...
  # Create forest plot of AMEs
  ame_plot <- ggplot(plot_data_ame, aes(x = DR_Severity, y = AME)) +
    geom_point(size = 4, color = "darkblue") +
    geom_errorbar(aes(ymin = CI_Lower, ymax = CI_Upper), 
                  width = 0.15, size = 1, color = "darkblue") +
    geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5, color = "gray50") +
    labs(title = "Average Marginal Effect of Lapsing on Vision Impairment by DR Severity",
         subtitle = "Change in probability of vision impairment (IPTW-weighted, with 95% CIs)",
         x = "DR Severity",
         y = "Average Marginal Effect (Probability Scale)") +
    theme_minimal() +
    theme(plot.title = element_text(size = 14, face = "bold"),
    ...
    geom_text(aes(label = paste0("n=", format(N, big.mark=","))),
              vjust = -2.5, size = 3.5, color = "gray50")
  
  print(ame_plot)
  
  # Create a second plot comparing AME magnitudes
