...
for(tx_type in c("no_treatment", "anti_VEGF", "PRP", "other_treatment")) {
  ...
  cat(paste("Processing", tx_type, "with n =", n_subset_tx, "\n"))
  
  if(n_subset_tx > 0) {
    # Try to calculate AME with error handling
    ame_calc_success <- tryCatch({
      # Create predictions with lapse = 0 and lapse = 1
      ...
      if("twang_att_w" %in% names(subset_data_tx)) {
        ...
        # Check lengths match
        if(length(individual_effects_tx) != length(subset_weights_tx)) {
          min_length <- min(length(individual_effects_tx), length(subset_weights_tx))
          individual_effects_tx <- individual_effects_tx[1:min_length]
          subset_weights_tx <- subset_weights_tx[1:min_length]
        }
      ...
      cat(paste("  Error calculating AME for", tx_type, ":", e$message, "\n"))
      FALSE
    })
  }
}
...
if(nrow(ame_results_tx) > 0) {
  ...
  cat("Could not calculate AMEs for treatment types due to prediction errors.\n")
  cat("This may be due to missing factor levels or data structure issues.\n")
}
```
