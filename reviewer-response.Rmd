...
# Check if we have probability table data with required columns
if(exists("prob_table") && nrow(prob_table) > 0 && 
   "Difference" %in% names(prob_table) && 
   all(c("CI_Lower", "CI_Upper") %in% names(prob_table)) &&
   !any(is.na(prob_table$Difference))) {
  # Use the emmeans-based differences for plotting
  plot_data_prob <- prob_table
  ...
  # Add sample size annotations with safe positioning
  y_position <- if(exists("plot_data_prob") && "CI_Lower" %in% names(plot_data_prob)) {
    min(plot_data_prob$CI_Lower, na.rm = TRUE) - 0.01
  } else {
    -0.05  # Default position
  }
  
  prob_diff_plot <- prob_diff_plot +
    annotate("text", x = 1, y = y_position, 
             label = "n=16,130", size = 3.5, color = "gray50") +
    annotate("text", x = 2, y = y_position, 
             label = "n=5,695", size = 3.5, color = "gray50") +
    annotate("text", x = 3, y = y_position, 
             label = "n=3,187", size = 3.5, color = "gray50")
  
  print(prob_diff_plot)
} else {
  cat("Skipping probability differences plot - required data not available from emmeans calculation.\n")
  if(exists("prob_table")) {
    cat("Available columns in prob_table:", paste(names(prob_table), collapse=", "), "\n")
  }
  cat("See manual calculation results for probability differences.\n")
}
```
