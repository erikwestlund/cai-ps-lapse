...
# Calculate marginal effects for DR severity model
# Marginal effect of lapsing at each DR severity level
dr_margins <- margins(model_dr_interaction, 
                     variables = "ever_lapse_binary",
                     at = list(person_dr = c(0, 1, 2)),
                     design = survey_design_weighted)

# Convert to data frame for plotting
dr_marginal_effects <- summary(dr_margins)
dr_marginal_effects$dr_label <- factor(dr_marginal_effects$person_dr,
                                      levels = c(0, 1, 2),
                                      labels = c("0: No DR", "1: NPDR", "2: PDR"))

# Calculate marginal effects for treatment type model
# Marginal effect of lapsing for each treatment type
tx_margins <- margins(model_treatment_interaction,
                      variables = "ever_lapse_binary", 
                      at = list(treatment_type = levels(m_data$treatment_type)),
                      design = survey_design_weighted)

# Convert to data frame for plotting  
tx_marginal_effects <- summary(tx_margins)

# Display marginal effects tables
cat("### DR Severity - Average Marginal Effects of Lapsing\n")
knitr::kable(dr_marginal_effects[, c("dr_label", "AME", "SE", "lower", "upper", "p")],
             caption = "Marginal Effect of Lapsing by DR Severity",
             digits = 4)

cat("\n### Treatment Type - Average Marginal Effects of Lapsing\n")
knitr::kable(tx_marginal_effects[, c("treatment_type", "AME", "SE", "lower", "upper", "p")],
...
```{r forest_plot, fig.height=8, fig.width=10}
...
combined_effects <- rbind(
  dr_marginal_effects[, c("Group", "Subgroup", "AME", "SE", "lower", "upper", "p")],
  tx_marginal_effects[, c("Group", "Subgroup", "AME", "SE", "lower", "upper", "p")]
)
