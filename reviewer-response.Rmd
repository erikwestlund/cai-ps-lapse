...
extract_predictions <- function(model, data, interaction_var, fixed_vars = NULL) {
  # Build prediction dataset manually
  if(interaction_var == "person_dr") {
    # Note: In the model formula, person_dr is wrapped in factor()
    # So we need to match the factor levels exactly as the model sees them
    # Get actual person_dr values and ensure they're treated as factors
    dr_values <- sort(unique(data$person_dr))
  ...
  # Get predictions - be very careful with survey model output
  tryCatch({
    ...
    if(inherits(preds, "svystat")) {
      ...
      # For svystat objects, try to extract variance
      # But be careful - it might not be the right dimension
      tryCatch({
        pred_var <- attr(preds, "var")
        if(!is.null(pred_var) && is.matrix(pred_var)) {
          # Only use diagonal if dimensions match
          if(nrow(pred_var) == length(pred_values) && ncol(pred_var) == length(pred_values)) {
            pred_se <- sqrt(diag(pred_var))
          } else {
            # Use a default SE
            pred_se <- rep(0.05, length(pred_values))
          }
        } else {
          pred_se <- rep(0.05, length(pred_values))
        }
