...
## Probability Difference by Lapse Status

```{r probability_differences, fig.height=5, fig.width=8}
# Calculate the actual probabilities for lapsers vs non-lapsers
# This shows the DIFFERENCE IN PROBABILITY of vision impairment between groups
...
# Convert log odds effects to probability differences
...
plot_data_prob <- plot_data |>
  mutate(
    # This is the DIFFERENCE in probability between lapsers and non-lapsers
    Prob_Difference = Log_Odds * prob_scale_factor,
    CI_Lower_Prob = CI_Lower * prob_scale_factor,
    CI_Upper_Prob = CI_Upper * prob_scale_factor
  )

# Plot the probability differences
prob_diff_plot <- ggplot(plot_data_prob, aes(x = DR_Severity, y = Prob_Difference)) +
  geom_point(size = 5, color = "darkblue") +
  geom_errorbar(aes(ymin = CI_Lower_Prob, ymax = CI_Upper_Prob), 
  ...
  annotate("text", x = 1, y = min(plot_data_prob$CI_Lower_Prob) - 0.01, 
           label = "n=16,130", size = 3.5, color = "gray50") +
  annotate("text", x = 2, y = min(plot_data_prob$CI_Lower_Prob) - 0.01, 
           label = "n=5,695", size = 3.5, color = "gray50") +
  annotate("text", x = 3, y = min(plot_data_prob$CI_Lower_Prob) - 0.01, 
           label = "n=3,187", size = 3.5, color = "gray50")

print(prob_diff_plot)

# Display the probability differences in a table
prob_diff_table <- plot_data_prob |>
  mutate(
    Prob_Diff_Pct = sprintf("%.1f%%", Prob_Difference * 100),
    CI_Lower_Pct = sprintf("%.1f%%", CI_Lower_Prob * 100),
    CI_Upper_Pct = sprintf("%.1f%%", CI_Upper_Prob * 100),
    Effect = paste0(Prob_Diff_Pct, " (", CI_Lower_Pct, ", ", CI_Upper_Pct, ")")
  ) |>
  select(DR_Severity, Prob_Difference, Effect)

knitr::kable(prob_diff_table[, c("DR_Severity", "Effect")],
             caption = "Difference in Probability of Vision Impairment: Lapsers vs Non-Lapsers",
             col.names = c("DR Severity", "Probability Difference (95% CI)"),
             row.names = FALSE)

# Also show the raw probabilities
cat("\nFor context, the baseline probability of vision impairment (at intercept) is approximately:", 
    sprintf("%.1f%%", baseline_prob * 100), "\n")
```
