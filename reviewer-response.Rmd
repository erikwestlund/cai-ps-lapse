...
for(subset_name in names(data_subsets)) {
  ...
  cat("\n\n")
  cat("## Data Subset: ", toupper(subset_name), "\n")
  ...
  for(weight_type in c("unweighted", "weighted")) {
    ...
    cat("\n")
    cat("### Analysis Type: ", toupper(weight_type), "\n\n")
    ...
    cat("\n")
    cat("#### DR Severity Interaction Model\n\n")
    ...
    cat("\n")
    cat("**DR Severity Interaction P-values:**\n\n")
    ...
    cat("\n")
    cat("#### Treatment Type Interaction Model\n\n")
    ...
    cat("\n")
    cat("**Treatment Type Interaction P-values:**\n\n")
...
cat("\n\n")
cat("**NOTE:** Interaction p-values test whether the effect of lapsing differs ")
...
extract_predictions <- function(model, data, interaction_var, fixed_vars = NULL) {
  # Get unique values for the interaction variable
  if(interaction_var == "person_dr") {
    unique_vals <- sort(unique(data$person_dr))
    pred_data <- expand.grid(
      ever_lapse_binary = c(0, 1),
      person_dr = unique_vals,
      stringsAsFactors = FALSE
    )
  } else if(interaction_var == "treatment_type") {
    unique_vals <- levels(data$treatment_type)
    pred_data <- expand.grid(
      ever_lapse_binary = c(0, 1),
      treatment_type = unique_vals,
      stringsAsFactors = FALSE
    )
  } else {
    pred_data <- expand.grid(
  ...
  for(var in model_vars) {
    if(!var %in% names(pred_data) && var != "outcome_va_vi_binary") {
      if(var %in% names(data)) {
        if(is.factor(data[[var]]) || is.character(data[[var]])) {
          ...
          # For logical variables, use FALSE
          pred_data[[var]] <- FALSE
        }
      }
    }
  ...
  # Make sure factors have the right levels
  if("person_dr" %in% names(pred_data)) {
    pred_data$person_dr <- factor(pred_data$person_dr)
  }
  if("treatment_type" %in% names(pred_data)) {
    pred_data$treatment_type <- factor(pred_data$treatment_type, 
                                      levels = levels(data$treatment_type))
  }
  
  # Get predictions from survey model
  # Note: svyglm predict doesn't support se.fit, so we'll use the variance from svystat
  preds <- predict(model, newdata = pred_data, type = "response")
  
  # Extract predictions and variances
  if(inherits(preds, "svystat")) {
    pred_data$predicted <- as.numeric(preds)
    # Get standard errors from the variance-covariance matrix if available
    pred_var <- attr(preds, "var")
    if(!is.null(pred_var) && length(diag(pred_var)) == nrow(pred_data)) {
      pred_data$se <- sqrt(diag(pred_var))
    } else {
      # Use a conservative default SE
      pred_data$se <- 0.05
    }
  } else {
    # Handle as regular numeric vector
    pred_data$predicted <- as.numeric(preds)
    pred_data$se <- 0.05
  }
