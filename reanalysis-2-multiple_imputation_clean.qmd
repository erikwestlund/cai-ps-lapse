...
# Comorbidity indicators (binary, recoded to 0/1)
vars_comorbidity <- c(
...
dr_cols <- c("PDR", "NPDR", "No_DR")
for (col in setdiff(dr_cols, names(df_original))) {
  df_original[[col]] <- NA_integer_
}
if (!"person_dr" %in% names(df_original)) {
  df_original[["person_dr"]] <- NA_integer_
}
df_mice <- df_original |>
  select(any_of(all_vars), any_of(dr_cols)) |>
  mutate(
    # Create person_dr from DR columns if needed
    person_dr = case_when(
      !is.na(PDR) & PDR == 1 ~ 2L,
      !is.na(NPDR) & NPDR == 1 ~ 1L,
      !is.na(No_DR) & No_DR == 1 ~ 0L,
      TRUE ~ person_dr
    ),
    # Convert to ordered factor
    person_dr = factor(person_dr, levels = c(0L, 1L, 2L), 
                      labels = c("No_DR", "NPDR", "PDR"),
                      ordered = TRUE),
    # Recode comorbidities to 0/1
    Glaucoma_bef_hitplus = ifelse(Glaucoma_bef_hitplus == "Present", 1, 0),
    Otherretina_bef_hitplus = ifelse(Otherretina_bef_hitplus == "Present", 1, 0),
...
# Continuous variables - use 2l.pmm for clustering
...
# Mark cluster variable for 2-level imputation
# -2 indicates this is the cluster ID
pred_matrix[cluster_var, ] <- 0  # Don't predict the cluster ID

# For 2l.pmm variables, ensure cluster is marked
vars_2l <- names(methods)[methods == "2l.pmm"]
for (var in vars_2l) {
  pred_matrix[var, cluster_var] <- -2
}
