---
title: "Reanalysis Step 5: Main Effects Outcome Analysis"
...
force_refit_models <- TRUE  # Set to TRUE to re-fit all models
force_recalc_emmeans <- TRUE  # Set to TRUE to recalculate marginal means
...
```{r define-formula}
# Get the REANALYSIS formulas from functions.R
# Using reanalysis formulas that include dr_severity as a covariate
reanalysis_formulas <- get_analysis_formulas_reanalysis()

# Use the full formula with dr_severity
main_effects_formula <- reanalysis_formulas$full
```
...
- Adjustment variables: 
  - DR severity (`dr_severity`)
  - Baseline visual acuity (`baseline_VA_logMAR`)
...
lapse_effect <- pooled_results[pooled_results$term == "ever_lapse_binary1", ]
...
for (i in 1:n_imputations) {
  ...
  lapse_row <- model_summaries[[i]][model_summaries[[i]]$term == "ever_lapse_binary1", ]
...
```{r marginal-effects}
# Check if marginal means are cached
emmeans_cache_file <- file.path(reanalysis_data_dir, "emmeans_results_main_effects.rds")

if (file.exists(emmeans_cache_file) && !force_recalc_emmeans) {
  # Load cached results
  emmeans_cache <- readRDS(emmeans_cache_file)
  emmeans_list <- emmeans_cache$emmeans_list
  contrasts_list <- emmeans_cache$contrasts_list
  emmeans_loaded_from_cache <- TRUE
  emmeans_cache_location <- emmeans_cache_file
} else {
  # Calculate marginal means for each imputed dataset then pool
  emmeans_loaded_from_cache <- FALSE
  emmeans_list <- list()
  contrasts_list <- list()
  
  # Initialize progress tracking
  progress <- init_progress(n_imputations, "Calculating marginal means")
  
  for (i in 1:n_imputations) {
    # Update progress
    progress <- update_progress(progress)
    
    # Get the fitted model for this imputation
    ...
    # Calculate emmeans on the response scale
    # Provide data explicitly for svyglm models
    emm <- emmeans(model, 
                   ~ ever_lapse_binary,
                   type = "response",
    ...
    # Get contrast (difference in probabilities)
    contrast <- pairs(emm, reverse = TRUE)  # Lapsed - Not Lapsed
    
    emmeans_list[[i]] <- as.data.frame(emm)
    contrasts_list[[i]] <- as.data.frame(contrast)
  }
  
  # Save to cache
  emmeans_cache <- list(
    emmeans_list = emmeans_list,
    contrasts_list = contrasts_list,
    n_imputations = n_imputations,
    timestamp = Sys.time()
  )
  saveRDS(emmeans_cache, emmeans_cache_file)
  emmeans_cache_location <- emmeans_cache_file
}

# Pool predicted probabilities for each group
# Extract probabilities for not lapsed (ever_lapse_binary = 0)
prob_not_lapsed <- sapply(emmeans_list, function(x) {
  x[x$ever_lapse_binary == 0, "prob"][1]
})
se_not_lapsed <- sapply(emmeans_list, function(x) {
  x[x$ever_lapse_binary == 0, "SE"][1]
})

# Extract probabilities for lapsed (ever_lapse_binary = 1)
prob_lapsed <- sapply(emmeans_list, function(x) {
  x[x$ever_lapse_binary == 1, "prob"][1]
})
se_lapsed <- sapply(emmeans_list, function(x) {
  x[x$ever_lapse_binary == 1, "SE"][1]
})

# Pool using Rubin's rules for each group
# Not lapsed
pooled_prob_not_lapsed <- mean(prob_not_lapsed)
within_var_not_lapsed <- mean(se_not_lapsed^2)
between_var_not_lapsed <- var(prob_not_lapsed)
total_var_not_lapsed <- within_var_not_lapsed + between_var_not_lapsed * (1 + 1/n_imputations)
pooled_se_not_lapsed <- sqrt(total_var_not_lapsed)

# Lapsed
pooled_prob_lapsed <- mean(prob_lapsed)
within_var_lapsed <- mean(se_lapsed^2)
between_var_lapsed <- var(prob_lapsed)
total_var_lapsed <- within_var_lapsed + between_var_lapsed * (1 + 1/n_imputations)
pooled_se_lapsed <- sqrt(total_var_lapsed)

# Calculate pooled risk difference
risk_diff <- pooled_prob_lapsed - pooled_prob_not_lapsed

# For risk difference SE, extract from contrasts
# Note: contrasts from emmeans pairs() gives odds ratios, not risk differences
# So we calculate risk difference SE using delta method or from the individual SEs
risk_diff_var <- total_var_lapsed + total_var_not_lapsed  # Conservative approximation
risk_diff_se <- sqrt(risk_diff_var)

# Calculate FMI and df for risk difference
between_var_diff <- var(prob_lapsed - prob_not_lapsed)
within_var_diff <- risk_diff_var - between_var_diff * (1 + 1/n_imputations)
fmi <- (between_var_diff * (1 + 1/n_imputations)) / risk_diff_var
df_diff <- (n_imputations - 1) / fmi^2

# CI using t-distribution
t_crit <- qt(0.975, df_diff)
ci_lower <- risk_diff - t_crit * risk_diff_se
ci_upper <- risk_diff + t_crit * risk_diff_se

# Create results data frame
pooled_emmeans_results <- data.frame(
  Group = c("Not Lapsed", "Lapsed", "Risk Difference"),
  Estimate = c(pooled_prob_not_lapsed, pooled_prob_lapsed, risk_diff),
  SE = c(pooled_se_not_lapsed, pooled_se_lapsed, risk_diff_se),
  CI_Lower = c(
    pooled_prob_not_lapsed - qt(0.975, df_diff) * pooled_se_not_lapsed,
    pooled_prob_lapsed - qt(0.975, df_diff) * pooled_se_lapsed,
    ci_lower
  ),
  CI_Upper = c(
    pooled_prob_not_lapsed + qt(0.975, df_diff) * pooled_se_not_lapsed,
...
# Display results
kable(pooled_emmeans_results,
      caption = "Pooled marginal means and risk difference from emmeans",
      digits = 4,
      col.names = c("Group", "Probability", "SE", "Lower CI", "Upper CI"))
...
**Marginal means cache status:** `r ifelse(emmeans_loaded_from_cache, paste("Loaded from", emmeans_cache_location), paste("Newly calculated and saved to", emmeans_cache_location))`
...
results_to_save <- list(
  ...
  predicted_probabilities = NULL,  # Predicted probabilities from emmeans section (single imputation only)
...
- **Effect size**: The average marginal effect indicates that lapsing increases the probability of VI/blindness by `r round(pooled_ame$AME, 3)`
