...
force_recalc_ame <- TRUE  # Set to TRUE to force recalculation
...
if (file.exists(ame_cache_file) && !force_recalc_ame) {
  ...
  for (i in 1:n_imputations) {
    ...
    # the model already handled the clustering
    ame <- marginaleffects::avg_comparisons(
      model,
      variables = "ever_lapse_binary",
      type = "response",   # average difference in predicted probabilities
      newdata = imp_data,
      wts = "weights"
    )
...
if (!is.null(emm_results)) {
  ...
  # Display predictions
  kable(pred_df |> select(Lapse_Status, Predicted_Prob, SE, asymp.LCL, asymp.UCL), 
  ...
  # Calculate and display the contrast
  contrasts_lapse <- pairs(emm_results)
  contrasts_df <- as.data.frame(contrasts_lapse)
  
  # When emmeans is on response scale, pairs() gives odds ratios
  # Calculate risk difference from the predicted probabilities
  risk_diff <- pred_df$Predicted_Prob[pred_df$Lapse_Status == "Lapsed"][1] - 
               pred_df$Predicted_Prob[pred_df$Lapse_Status == "Not Lapsed"][1]
  
  # Get confidence intervals using confint if not present
  if (!("lower.CL" %in% names(contrasts_df))) {
    contrasts_ci <- confint(contrasts_lapse)
    contrasts_df <- cbind(contrasts_df, contrasts_ci[, c("asymp.LCL", "asymp.UCL")])
  }
  
  # Display odds ratio from contrast
  # Note: OR < 1 means lower odds in reference group (ever_lapse_binary0)
  # We want to show it as Lapsed vs Not Lapsed, so take reciprocal
  or_value <- 1 / contrasts_df$odds.ratio[1]  # Invert to get Lapsed/Not Lapsed
  
  or_display <- data.frame(
    Contrast = "Lapsed vs Not Lapsed",
    Odds_Ratio = round(or_value, 3),
    SE = round(contrasts_df$SE[1], 4),
    CI_Lower = round(1/contrasts_df$asymp.UCL[1], 3),  # Invert and swap
    CI_Upper = round(1/contrasts_df$asymp.LCL[1], 3),  # Invert and swap
    p.value = format.pval(contrasts_df$p.value[1], digits = 4, eps = 0.0001)
  )
  
  kable(or_display,
        caption = "Main effect: Odds ratio for lapse effect",
        col.names = c("Contrast", "Odds Ratio", "SE", "Lower CI", "Upper CI", "P-value"))
  
  # Also display risk difference
  risk_diff_df <- data.frame(
  ...
  kable(risk_diff_df,
        caption = "Main effect: Risk difference (from predicted probabilities)",
        col.names = c("Contrast", "Risk Difference"))
} else {
