...
imputation_vars <- c(
  ...
  "cohort_id",  # Indicates if true 2-year outcome available
...
custom_methods["cohort_id"] <- ""  # Never impute - complete indicator
...
# 3. Cohort_id can predict VA variables (indicates data structure)
pred_matrix["baseline_VA_logMAR", "cohort_id"] <- 1
pred_matrix["intermediate_VA_logMAR", "cohort_id"] <- 1
pred_matrix["outcome_VA_logMAR", "cohort_id"] <- 1

# 4. CRITICAL: Prevent reverse-time prediction
...
# Cohort_id should not be predicted by anything
pred_matrix["cohort_id", ] <- 0
