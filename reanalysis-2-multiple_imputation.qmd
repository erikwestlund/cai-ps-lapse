...
```{r setup}
#| include: false
# Setup analysis environment (includes seed setting)
...
# Variables to include in imputation model (following Rubin's guidance)
...
imputation_vars <- c(
  # Patient ID (not imputed, just kept)
  "e_mrn_deidentified",
  ...
  # Demographics/Clinical - SOURCE ONLY
  "baseline_VA_logMAR",
  "gender_cat",
  "race_ethnic_cat",
  "insurance_cat",
...
# Select only imputation variables
...
missing_pattern <- md.pattern(df_mice, rotate.names = TRUE)

# Summary of missingness
miss_summary <- df_mice |>
  miss_var_summary() |>
  filter(n_miss > 0)
...
gg_miss_upset(df_mice, nsets = 10)
```
...
message("=== IMPUTATION MODEL SETUP ===\n")
...
# Check default imputation methods
...
# Use predictive mean matching for continuous/count variables
...
custom_methods["outcome_VA_logMAR"] <- "pmm"  # If any missing
...
# Don't impute these variables (but they ARE still used as predictors!)
# Method = "" means: use as predictor but don't impute
custom_methods["e_mrn_deidentified"] <- ""  # ID - never impute
...
# Note: Variables with method = "" are STILL USED as predictors for other variables
# They just aren't imputed themselves
...
# Summary of methods
method_types <- ifelse(custom_methods == "", "Predictor only (not imputed)",
                ifelse(custom_methods == "pmm", "Predictive mean matching",
                ifelse(custom_methods == "polr", "Proportional odds regression",
                ifelse(custom_methods == "logreg", "Logistic regression",
                ifelse(custom_methods == "polyreg", "Polytomous regression",
                       custom_methods)))))
...
final_methods <- final_methods[order(final_methods$Method == "", final_methods$Variable), ]

message("Imputation methods by variable:")
print(final_methods, n = 50)

# Show which variables will be imputed
vars_to_impute <- names(custom_methods)[custom_methods != ""]
message("\nVariables that WILL be imputed (", length(vars_to_impute), " total):")
...
1. **Propensity Score Analysis** (`reanalysis-3-propensity_scores.qmd`): 
   - Calculate propensity scores on each imputed dataset
   - Apply matching/weighting methods
   
2. **Outcome Analysis** (`reanalysis-4-outcome_analysis.qmd`):
   - Fit outcome models on each dataset
   - Pool results using Rubin's rules
   
3. **Sensitivity Analysis** (`reanalysis-5-sensitivity.qmd`):
   - Compare with complete case analysis
   - Assess impact of imputation on results