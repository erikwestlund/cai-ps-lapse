...
custom_methods["row_id"] <- ""  # Row ID - never impute, never use as predictor
custom_methods["e_mrn_deidentified"] <- ""  # Patient ID - never impute, never use as predictor
...
**Variables that WILL be imputed** (`r length(vars_to_impute)` total):
```{r vars-to-impute}
print(vars_to_impute)

# Show predictor matrix summary
avg_predictors <- sum(pred_matrix[1,])
predictors <- names(which(colSums(pred_matrix) > 0))
```

### Predictor Matrix Summary

Each variable uses an average of **`r avg_predictors`** predictors.
