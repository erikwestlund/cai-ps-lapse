...
# n_eyes can be predictor but not predicted
if ("n_eyes" %in% rownames(pred_matrix)) {
  pred_matrix["n_eyes", ] <- 0  # Not predicted
  # n_eyes can help predict missingness patterns
  # Leave default (1) for using as predictor
}
...
imputed_datasets <- lapply(imputed_datasets, function(imp_data) {
  ...
  patient_level_vars <- imp_data |>
    group_by(e_mrn_deidentified) |>
    summarise(
      person_dr_consistent = max(person_dr, na.rm = TRUE),
      CCI_consistent = max(CCI, na.rm = TRUE),
      DCSI_consistent = max(DCSI, na.rm = TRUE),
      .groups = 'drop'
    )
