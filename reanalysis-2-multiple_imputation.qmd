...
library(mice)
library(miceadds)  # For additional 2-level imputation methods
library(VIM)
library(naniar)
...
imputation_vars <- c(
  ...
  "fellow_outcome_VA",
  # "fellow_lapsed",  # REMOVED: Collinear with ever_lapse_binary
  "fellow_dr",
  "n_eyes",
  # "has_fellow",  # REMOVED: Perfectly collinear with n_eyes (has_fellow = n_eyes == 2)
  "patient_worst_baseline",
  "patient_best_baseline"
  # "patient_ever_lapsed"  # REMOVED: Collinear with ever_lapse_binary
)
...
df_mice <- df_for_imputation |>
  ...
  mutate(
    # Clean factor levels to remove spaces and special characters for 2l.pmm compatibility
    # Spaces in factor levels cause formula parsing errors in lmer
    # Replace spaces and special characters with underscores
    across(where(is.factor), ~ {
      if (is.factor(.x)) {
        factor(.x, labels = gsub("[^[:alnum:]_]", "_", levels(.x)))
      } else {
        .x
      }
    })
...
# Debug: Check if any methods have invalid values
# Include all standard MICE methods plus 2-level methods from mice and miceadds
valid_methods <- c("", "pmm", "logreg", "polyreg", "polr", "norm", "norm.boot", "norm.nob", 
                   "norm.predict", "sample", "cart", "rf", 
                   # 2-level methods
                   "2l.norm", "2l.lmer", "2l.pan", "2lonly.mean", "2lonly.norm", "2lonly.pmm",
                   # miceadds methods
                   "2l.pmm", "2l.2stage.norm", "2l.2stage.pmm", "ml.lmer")
invalid_methods <- custom_methods[!custom_methods %in% valid_methods]
if (length(invalid_methods) > 0) {
  log_message(paste0("WARNING: Invalid imputation methods detected: ", paste(names(invalid_methods), "=", invalid_methods, collapse=", ")))
  # Reset any invalid methods to appropriate defaults
  for (var in names(invalid_methods)) {
    if (var %in% names(custom_methods)) {
      # Set to empty string to skip imputation for now
      custom_methods[var] <- ""
      log_message(paste0("Reset method for ", var, " to '' (no imputation)"))
    }
  }
}

# Ensure methods vector has proper names
if (is.null(names(custom_methods))) {
  log_message("ERROR: custom_methods has no names. Setting names from df_mice columns.")
  names(custom_methods) <- colnames(df_mice)
}

# Print diagnostic info
log_message(paste0("Number of variables in custom_methods: ", length(custom_methods)))
log_message(paste0("First few method assignments: ", 
                  paste(head(names(custom_methods)), "=", head(custom_methods), collapse=", ")))

# Temporal VA variables - use 2-level PMM for clustered data
# 2l.pmm from miceadds: Multilevel PMM using linear mixed models
# This properly accounts for correlation between eyes from same patient
custom_methods["baseline_VA_logMAR"] <- "2l.pmm"  # Multilevel PMM for eyes within patients
custom_methods["intermediate_VA_logMAR"] <- "2l.pmm"  # Multilevel PMM for eyes within patients  
custom_methods["outcome_VA_logMAR"] <- "2l.pmm"  # Multilevel PMM for eyes within patients
custom_methods["cohort_id"] <- ""  # Never impute - data management indicator only

# Other continuous variables - use 2l.pmm for clustered data
custom_methods["CCI"] <- "2l.pmm"  # Multilevel PMM
custom_methods["DCSI"] <- "2l.pmm"  # Multilevel PMM
custom_methods["age"] <- "pmm"  # Regular PMM (patient-level, doesn't vary by eye)
...
# Mark e_mrn_deidentified as cluster variable
# -2 indicates this is the cluster ID (patient ID) for 2-level imputation
# This is REQUIRED for 2l.pmm to work properly
# The 2l.pmm method from miceadds:
# - Uses linear mixed models (lmer) to model the multilevel structure
# - Preserves within-patient correlation between eyes
# - Obtains PMM donors across all clusters by default
# - Properly accounts for the hierarchical data structure
pred_matrix[, "e_mrn_deidentified"] <- -2
pred_matrix["e_mrn_deidentified", ] <- 0  # Don't predict the cluster ID itself
...
# For 2l.pmm variables, use a selective set of predictors to avoid formula length issues
# Identify which variables will use 2l.pmm
vars_2l_pmm <- c("baseline_VA_logMAR", "intermediate_VA_logMAR", "outcome_VA_logMAR", "CCI", "DCSI")

# Define core predictors for 2l.pmm models (keep it focused)
core_predictors_2l <- c(
  "ever_lapse_binary",  # Exposure
  "age",                # Key demographic
  "gender_cat",         # Gender
  "baseline_VA_logMAR", # Baseline VA (for temporal)
  "intermediate_VA_logMAR", # Intermediate (for outcome)
  "person_dr",          # DR severity
  "anti_VEGF",          # Key treatment
  "CCI", "DCSI",       # Comorbidities
  "fellow_baseline_VA", # Fellow eye for correlation
  "fellow_outcome_VA"   # Fellow eye outcome
)

# For 2l.pmm variables, limit predictors to core set
log_message("Configuring predictor matrix for 2l.pmm variables with limited predictors")
for (var in vars_2l_pmm) {
  if (var %in% rownames(pred_matrix)) {
    # First set all to 0
    pred_matrix[var, ] <- 0
    # Then selectively enable core predictors
    n_predictors <- 0
    for (pred in core_predictors_2l) {
      if (pred %in% colnames(pred_matrix) && pred != var) {
        pred_matrix[var, pred] <- 1
        n_predictors <- n_predictors + 1
      }
    }
    log_message(paste0("  ", var, ": ", n_predictors, " predictors"))
  }
}

# Ensure exposure is used as predictor for all outcomes
...
if ("fellow_outcome_VA" %in% colnames(pred_matrix)) {
  ...
  # Only set for variables that actually exist in the matrix
  fellow_vars_in_matrix <- c("fellow_outcome_VA", "fellow_baseline_VA", "fellow_intermediate_VA", "fellow_dr")
  fellow_vars_in_matrix <- fellow_vars_in_matrix[fellow_vars_in_matrix %in% rownames(pred_matrix)]
  if (length(fellow_vars_in_matrix) > 0) {
    pred_matrix[fellow_vars_in_matrix, ] <- 0
  }
  
  # Don't let fellow eye variables be their own predictors
  if ("fellow_outcome_VA" %in% rownames(pred_matrix)) {
    pred_matrix["fellow_outcome_VA", "fellow_outcome_VA"] <- 0
  }
  if ("fellow_baseline_VA" %in% rownames(pred_matrix)) {
    pred_matrix["fellow_baseline_VA", "fellow_baseline_VA"] <- 0
  }
  if ("fellow_intermediate_VA" %in% rownames(pred_matrix)) {
...
if ("patient_worst_baseline" %in% colnames(pred_matrix)) {
  ...
  # pred_matrix[, "patient_ever_lapsed"] <- 1  # REMOVED - variable not in imputation
  
  # But don't predict these variables
  pred_matrix["patient_worst_baseline", ] <- 0
  pred_matrix["patient_best_baseline", ] <- 0
  # pred_matrix["patient_ever_lapsed", ] <- 0  # REMOVED - variable not in imputation
}

# n_eyes is just an indicator, not a predictor
if ("n_eyes" %in% colnames(pred_matrix)) {
  pred_matrix[, "n_eyes"] <- 0
  pred_matrix["n_eyes", ] <- 0
  # has_fellow removed from imputation - was collinear with n_eyes
}
...
# Final check on methods before running MICE
log_message("Final method check before imputation:")
methods_with_values <- custom_methods[custom_methods != ""]
log_message(paste0("Variables to be imputed: ", paste(names(methods_with_values), collapse=", ")))
log_message(paste0("Methods to be used: ", paste(unique(methods_with_values), collapse=", ")))
...
#    This is EXPECTED and HARMLESS - they measure different eyes but are missing for same structural reason
#    MICE will drop fellow_outcome_VA from the imputation model for outcome_VA_logMAR to avoid
#    singularity, but it can still be used to predict other variables. This does NOT affect results.
# 
# Variables removed to avoid true collinearity: has_fellow, patient_ever_lapsed, fellow_lapsed
...
# Save convergence plot for later diagnostics
pdf(file.path(reanalysis_data_dir, "mice_convergence.pdf"))
plot(mice_results)
dev.off()

log_message("Convergence plot saved to mice_convergence.pdf")
```
