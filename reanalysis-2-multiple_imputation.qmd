...
# FINAL STEP: Enforce person_dr consistency across all datasets
# Take the maximum person_dr value per patient to ensure consistency
imputed_datasets <- lapply(imputed_datasets, function(imp_data) {
  # Calculate max person_dr per patient
  person_dr_max_lookup <- imp_data |>
    group_by(e_mrn_deidentified) |>
    summarise(
      person_dr_max_value = max(person_dr, na.rm = TRUE),
      .groups = 'drop'
    )
  
  # Replace person_dr with the consistent max value
  imp_data <- imp_data |>
    left_join(person_dr_max_lookup, by = "e_mrn_deidentified") |>
    mutate(person_dr = person_dr_max_value) |>
    select(-person_dr_max_value)
  
  return(imp_data)
})

log_message("Enforced person_dr consistency by taking max value per patient")
...
### Person-Level DR Consistency Check

```{r dr-consistency-check}
# Check that person_dr is consistent within patients after imputation
dr_consistency_check <- lapply(1:n_imputations, function(i) {
  imp_data <- imputed_datasets[[i]]
  
  # Check consistency of person_dr within patients
  dr_check <- imp_data |>
    group_by(e_mrn_deidentified) |>
    summarise(
      n_eyes = n(),
      unique_dr_values = n_distinct(person_dr, na.rm = TRUE),
      is_consistent = unique_dr_values <= 1  # Should be 1 unique value per patient
    )
  
  n_inconsistent <- sum(!dr_check$is_consistent)
  return(list(
    dataset = i,
    n_patients_checked = nrow(dr_check),
    n_inconsistent_patients = n_inconsistent,
    pct_consistent = round((1 - n_inconsistent/nrow(dr_check)) * 100, 2)
  ))
})

# Display results
dr_consistency_df <- bind_rows(dr_consistency_check)
log_message(paste0("DR consistency check: ", 
                  min(dr_consistency_df$pct_consistent), "% to ",
                  max(dr_consistency_df$pct_consistent), "% consistent across datasets"))
```

**Person-level DR consistency after imputation:**
```{r dr-consistency-table}
kable(dr_consistency_df, 
      col.names = c("Dataset", "Patients", "Inconsistent", "% Consistent"))
```
