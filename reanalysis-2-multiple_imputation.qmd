...
df_mice <- df_for_imputation |>
  ...
  dplyr::select(row_id, cluster_id, dplyr::all_of(imputation_vars), person_dr_max) |>
  mutate(
...
imputed_datasets <- lapply(1:n_imputations, function(i) {
  ...
  # CRITICAL: Copy person_dr_max (imputed) to person_dr for create_dr_severity_variables
  # person_dr_max is the person-level DR that was imputed
  # person_dr needs to be set for each eye from person_dr_max
  if ("person_dr_max" %in% names(imp_data)) {
    # Convert from factor if needed
    if (is.factor(imp_data$person_dr_max)) {
      # Convert factor levels (No_DR=1, NPDR=2, PDR=3) to numeric (0, 1, 2)
      imp_data$person_dr <- as.numeric(imp_data$person_dr_max) - 1
    } else {
      imp_data$person_dr <- imp_data$person_dr_max
    }
  }
