...
imputed_datasets <- lapply(1:n_imputations, function(i) {
  ...
  # Copy person_dr_max back to person_dr for each eye
  # This ensures DR severity is consistent within patients
  # Handle the factor conversion correctly
  imp_data <- imp_data |>
    mutate(
      # person_dr_max is an ordered factor with levels "No_DR", "NPDR", "PDR"
      # Convert to numeric 0, 1, 2
      person_dr = case_when(
        person_dr_max == "No_DR" ~ 0,
        person_dr_max == "NPDR" ~ 1,
        person_dr_max == "PDR" ~ 2,
        TRUE ~ NA_real_
      )
    ) |>
    select(-person_dr_max)  # Remove person_dr_max after using it
