...
`cohort_id` is included in the dataset but NOT used as a predictor in the imputation model to avoid perfect prediction issues. The variable is set to never be imputed (method = "") and its column in the predictor matrix is set to 0 so it cannot predict other variables. This allows mice to understand the data structure while preventing perfect prediction problems.
:::
...
imputation_vars <- c(
  ...
  # Data indicator - will NOT be imputed or used as predictor
  "cohort_id",                 # Kept in dataset but not used for prediction
  ...
  "person_dr"                  # Numeric 0/1/2 → ordered factor
...
df_mice <- df_for_imputation |>
  ...
  mutate(
    ...
    cohort_id = factor(cohort_id),  # Keep as factor but won't impute
...
# Data indicator - never impute
custom_methods["cohort_id"] <- ""
...
for (idv in c("row_id", "e_mrn_deidentified")) {
  if (idv %in% rownames(pred_matrix)) pred_matrix[idv, ] <- 0
  if (idv %in% colnames(pred_matrix)) pred_matrix[, idv] <- 0
}

# Never impute these variables (set their rows to 0)
if ("ever_lapse_binary" %in% rownames(pred_matrix)) pred_matrix["ever_lapse_binary", ] <- 0
if ("cohort_id" %in% rownames(pred_matrix)) pred_matrix["cohort_id", ] <- 0

# CRITICAL: cohort_id must NOT be used as a predictor (set its column to 0)
if ("cohort_id" %in% colnames(pred_matrix)) pred_matrix[, "cohort_id"] <- 0
...
if ("outcome_VA_logMAR" %in% names(df_mice)) {
  ...
  # Check cohort_id setup
  if ("cohort_id" %in% names(df_mice)) {
    log_message("  - cohort_id: included but NOT used as predictor")
    cohort_excluded <- sum(df_mice$cohort_id == "excluded")
    log_message(paste0("  - Records with cohort_id='excluded': ", cohort_excluded))
  }
...
# Check specific key variables
key_vars_check <- c("baseline_VA_logMAR", "intermediate_VA_logMAR", "outcome_VA_logMAR", "person_dr")
log_message("\nMissing values in key variables after imputation:")
for (v in key_vars_check) {
  if (v %in% names(first_imp)) {
    n_miss_after <- sum(is.na(first_imp[[v]]))
    n_miss_before <- sum(is.na(df_mice[[v]]))
    log_message(paste0(v, ": Before=", n_miss_before, ", After=", n_miss_after))
  }
}

# Report on variables that still have missing values
missing_after <- missing_after[missing_after > 0]
if (length(missing_after) > 0) {
  log_message("\nWARNING: Variables with remaining missing values:")
  print(missing_after)
  
  # Check if these are the fellow variables (which shouldn't be imputed)
  fellow_vars_missing <- intersect(names(missing_after), 
                                   c("fellow_baseline_VA", "fellow_intermediate_VA", 
                                     "fellow_outcome_VA", "fellow_dr"))
  if (length(fellow_vars_missing) > 0) {
    log_message("Note: Fellow variables are expected to have missing values (single eyes)")
  }
} else {
  log_message("\nSUCCESS: All missing data imputed in first completed dataset.")
}
```
...
# Add back variables not included in imputation or transformations
df_original_full <- df_for_imputation |>
  mutate(row_id = dplyr::row_number())

# Add any other variables not included in imputation or transformations
