...
# CACHING CONFIGURATION FOR MICE
# Set to TRUE to use cached MICE results (skip imputation, only run post-processing)
use_cached_mice <- FALSE
mice_cache_file <- file.path(reanalysis_data_dir, "mice_results.rds")
log_message(paste0("MICE caching: ", ifelse(use_cached_mice, "ENABLED", "DISABLED")))
...
# Prepare data for MICE - NO person_dr_max, use eye-level person_dr
df_mice <- df_for_imputation |>
  mutate(
    # Create a unique row identifier to preserve exact row matching
    row_id = row_number(),
    # Create numeric cluster ID for 2l.pan (required for multilevel imputation)
    cluster_id = as.numeric(factor(e_mrn_deidentified))
  ) |>
  dplyr::select(row_id, cluster_id, dplyr::all_of(imputation_vars)) |>
  mutate(
    # Convert person_dr to ordered factor for polr imputation method
    # person_dr is eye-level DR severity (0=No_DR, 1=NPDR, 2=PDR)
...
log_message(paste0("person_dr (eye-level): ", 
                  sum(!is.na(df_mice$person_dr)), " non-missing, ",
                  sum(is.na(df_mice$person_dr)), " missing"))
```
...
# Temporal VA variables - use 2l.pan for clustered imputation
custom_methods["baseline_VA_logMAR"] <- "2l.pan"  # Clustered imputation
custom_methods["intermediate_VA_logMAR"] <- "2l.pan"  # Clustered imputation
custom_methods["outcome_VA_logMAR"] <- "2l.pan"  # Clustered imputation
...
# person_dr is the eye-level DR severity
...
custom_methods["cluster_id"] <- ""  # Cluster ID for 2l methods - never impute
...
# EXCEPT: cluster_id is set to -2 as cluster indicator for 2l methods
pred_matrix[, "cluster_id"] <- -2  # Cluster indicator for 2l.pan
pred_matrix["cluster_id", ] <- 0  # Don't predict cluster_id itself
pred_matrix[, "e_mrn_deidentified"] <- 0  # Don't use patient ID as predictor
pred_matrix["e_mrn_deidentified", ] <- 0  # Don't predict patient ID
...
# Handle person_dr (eye-level DR severity)
# person_dr should be used as predictor and can be predicted
pred_matrix[, "person_dr"] <- 1  # Use as predictor for most variables
pred_matrix["person_dr", "person_dr"] <- 0  # Not its own predictor
pred_matrix["baseline_VA_logMAR", "person_dr"] <- 0  # Don't use to predict baseline VA
pred_matrix["row_id", "person_dr"] <- 0  # Don't use for IDs
pred_matrix["e_mrn_deidentified", "person_dr"] <- 0  # Don't use for IDs
pred_matrix["cohort_id", "person_dr"] <- 0  # Don't use for cohort_id
...
# Check for perfect collinearity between intermediate and outcome for submitted cohort
collinearity_check <- df_mice |>
  filter(cohort_id == "submitted") |>
  summarise(
    n = n(),
    n_with_both = sum(!is.na(intermediate_VA_logMAR) & !is.na(outcome_VA_logMAR)),
    cor_intermediate_outcome = cor(intermediate_VA_logMAR, outcome_VA_logMAR, use = "complete.obs"),
    are_identical = all(intermediate_VA_logMAR == outcome_VA_logMAR, na.rm = TRUE)
  )

log_message(paste0("Submitted cohort - Correlation between intermediate and outcome: ",
                  round(collinearity_check$cor_intermediate_outcome, 3)))
log_message(paste0("Submitted cohort - Are intermediate and outcome identical? ",
                  collinearity_check$are_identical))

# Log which predictors are set for outcome_VA_logMAR
outcome_predictors <- names(which(pred_matrix["outcome_VA_logMAR", ] == 1))
log_message(paste0("Predictors for outcome_VA_logMAR (", length(outcome_predictors), "): ",
...
- MICE caching: `r ifelse(use_cached_mice, "ENABLED", "DISABLED")`
- Cache file: `r if(use_cached_mice && file.exists(mice_cache_file)) "EXISTS (will load)" else "NOT FOUND (will run fresh)"`

```{r mice-run}
# Check for cached MICE results
if (use_cached_mice && file.exists(mice_cache_file)) {
  log_message("Loading cached MICE results...")
  log_message(paste0("Cache file: ", mice_cache_file))
  
  mice_results <- readRDS(mice_cache_file)
  
  # Verify cached results match expected parameters
  cached_m <- mice_results$m
  cached_maxit <- mice_results$iteration
  
  log_message(paste0("Loaded ", cached_m, " imputations from cache"))
  log_message(paste0("Original run used ", cached_maxit, " iterations"))
  
  if (cached_m != n_imputations) {
    warning(paste0("Cached imputations (", cached_m, ") differ from requested (", n_imputations, ")"))
  }
  
  imp_time <- 0  # No time spent on imputation
  log_message("Using cached MICE results - skipping imputation step")
  
} else {
  # Run fresh MICE imputation
  if (use_cached_mice && !file.exists(mice_cache_file)) {
    log_message("Cache file not found - running fresh imputation")
  } else {
    log_message("Cache disabled - running fresh imputation")
  }
  ...
  saveRDS(mice_results, mice_cache_file)
  log_message(paste0("Saved MICE results to cache: ", mice_cache_file))
}

# Log person_dr imputation status
if ("person_dr" %in% names(mice_results$imp)) {
  n_imputed_dr <- sum(mice_results$nmis["person_dr"])
  log_message(paste0("person_dr imputed for ", n_imputed_dr, " observations"))
}
...
imputed_datasets <- lapply(1:n_imputations, function(i) {
  ...
  # Convert person_dr from factor back to numeric if needed
  if (is.factor(imp_data$person_dr)) {
    # Convert factor levels (No_DR=1, NPDR=2, PDR=3) to numeric (0, 1, 2)
    imp_data$person_dr <- as.numeric(imp_data$person_dr) - 1
  }
  ...
  # Verify dr_severity was created properly
  if (!"dr_severity" %in% names(imp_data)) {
    stop("dr_severity not created in imputation ", i)
  }
  if (!is.factor(imp_data$dr_severity)) {
    stop("dr_severity is not a factor in imputation ", i)
  }
...
imputed_datasets <- lapply(imputed_datasets, function(imp_data) {
  ...
  patient_level_vars <- imp_data |>
    ...
    summarise(
      # For CCI and DCSI, take minimum (better value) per patient
      ...
      DCSI_consistent = ifelse(all(is.na(DCSI)), 
                              NA_real_, 
                              min(DCSI, na.rm = TRUE)),
      # Note: person_dr is now eye-level, so we don't enforce consistency
      .groups = 'drop'
    )
  
  # Replace with consistent values (CCI and DCSI only, not person_dr)
  imp_data <- imp_data |>
    left_join(patient_level_vars, by = "e_mrn_deidentified") |>
    mutate(
      # Only replace CCI and DCSI with consistent (minimum) values
      CCI = coalesce(CCI_consistent, CCI),
      DCSI = coalesce(DCSI_consistent, DCSI)
      # person_dr remains eye-level, not enforced to be consistent
    ) |>
    select(-CCI_consistent, -DCSI_consistent)
  
  return(imp_data)
})

log_message("Enforced patient-level consistency for CCI and DCSI by taking minimum (better) value per patient")
...
```{r comprehensive-completeness}
# Define key variables that should have been imputed
key_imputed_vars <- c(
  ...
  # DR severity (derived from person_dr)
  "person_dr",
  "dr_severity",
...
# Check completeness for each imputation
completeness_summary <- lapply(1:min(5, n_imputations), function(i) {
  imp_data <- imputed_datasets[[i]]
  
  # Check each key variable
  var_status <- lapply(key_imputed_vars, function(v) {
    if (v %in% names(imp_data)) {
      n_missing <- sum(is.na(imp_data[[v]]))
      pct_missing <- round(100 * n_missing / nrow(imp_data), 2)
      data.frame(
        Variable = v,
        Imputation = i,
        N_Missing = n_missing,
        Pct_Missing = pct_missing,
        Status = ifelse(n_missing == 0, "Complete", paste0(n_missing, " missing"))
      )
    } else {
      data.frame(
        Variable = v,
  ...
  do.call(rbind, var_status)
})

# Combine all results
completeness_df <- do.call(rbind, completeness_summary)

# Create summary table by variable across imputations
completeness_wide <- completeness_df |>
  select(Variable, Imputation, N_Missing) |>
  pivot_wider(names_from = Imputation, 
              values_from = N_Missing,
              names_prefix = "Imp_")

kable(completeness_wide, 
      caption = "Missing values by variable across first 5 imputations (should all be 0)",
      align = c('l', rep('r', min(5, n_imputations))))

# Flag any variables with remaining missing data
problem_vars <- completeness_df |>
  filter(N_Missing > 0) |>
  select(Variable, Imputation, N_Missing) |>
  unique()

if (nrow(problem_vars) > 0) {
  cat("\n⚠️ WARNING: The following variables still have missing data after imputation:\n")
  kable(problem_vars)
} else {
  cat("\n✓ All key variables are complete across all checked imputations\n")
}

# Compare before and after imputation
cat("\n### Before vs After Imputation Summary\n")

original_missing <- mice_results$nmis[key_imputed_vars[key_imputed_vars %in% names(mice_results$nmis)]]
post_imp_missing <- completeness_df |>
  filter(Imputation == 1) |>
  group_by(Variable) |>
  summarise(Post_Missing = sum(N_Missing, na.rm = TRUE)) |>
  filter(Variable %in% names(original_missing))

comparison_df <- data.frame(
  Variable = names(original_missing),
  Original_Missing = original_missing,
  Post_Imputation_Missing = 0  # Will be updated if any found
)

# Update with actual post-imputation values if available
for (v in comparison_df$Variable) {
  if (v %in% post_imp_missing$Variable) {
    comparison_df[comparison_df$Variable == v, "Post_Imputation_Missing"] <- 
      post_imp_missing[post_imp_missing$Variable == v, "Post_Missing"]
  }
}

comparison_df$Status <- ifelse(comparison_df$Post_Imputation_Missing == 0, 
                               "✓ Complete", 
                               "⚠️ Still missing")

kable(comparison_df,
      caption = "Imputation effectiveness: Original vs Post-imputation missing counts",
      col.names = c("Variable", "Original Missing", "After Imputation", "Status"),
      row.names = FALSE)
```
...
### DR Severity Variable Check

```{r dr-severity-check}
# Verify dr_severity was created properly in all datasets
dr_severity_summary <- lapply(1:min(3, n_imputations), function(i) {
  imp_data <- imputed_datasets[[i]]
  
  # Check dr_severity exists and is a factor
  if (!"dr_severity" %in% names(imp_data)) {
    return(data.frame(imputation = i, status = "MISSING"))
  }
  
  # Get summary
  dr_table <- table(imp_data$dr_severity, useNA = "ifany")
  
  data.frame(
    imputation = i,
    No_DR = dr_table["No_DR"],
    NPDR = dr_table["NPDR"],
    PDR = dr_table["PDR"],
    Missing = sum(is.na(imp_data$dr_severity)),
    is_factor = is.factor(imp_data$dr_severity),
    n_levels = nlevels(imp_data$dr_severity)
  )
})

dr_summary_df <- do.call(rbind, dr_severity_summary)
kable(dr_summary_df, caption = "DR severity distribution in first 3 imputed datasets")

# Confirm factor levels are correct
if (is.factor(imputed_datasets[[1]]$dr_severity)) {
  cat("DR severity levels:", paste(levels(imputed_datasets[[1]]$dr_severity), collapse = ", "), "\n")
}
```
...
```{r save-imputed}
# Only save mice_results if it wasn't loaded from cache (to avoid overwriting good data)
if (!use_cached_mice || !file.exists(mice_cache_file)) {
  saveRDS(mice_results, file.path(reanalysis_data_dir, "mice_results.rds"))
  log_message("Saved fresh MICE results to mice_results.rds")
} else {
  log_message("Skipping mice_results.rds save (using cached version)")
}

# Always save the post-processed imputed datasets (these have our fixes applied)
...
log_message("Saved post-processed imputed datasets")
```
