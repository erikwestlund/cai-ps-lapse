...
df_with_fellow <- df_with_fellow |>
  ...
  mutate(
    ...
    fellow_intermediate_VA = if_else(n_eyes == 2,
                                    ...
                                    NA_real_)
    # NOTE: fellow_outcome_VA NOT created - has identical missingness with outcome_VA_logMAR
    # This causes collinearity and makes mice drop intermediate_VA as predictor
    # fellow_dr also not needed since person_dr is already consistent within patients
  ) |>
  ungroup() |>
...
imputation_vars <- c(
  ...
  # NOTE: fellow_outcome_VA excluded - has identical missingness to outcome_VA_logMAR
  # Including it causes collinearity issues and drops important predictors
...
fellow_vars <- c("fellow_baseline_VA", "fellow_intermediate_VA")
...
vars_to_impute <- final_methods |>
  ...
  filter(!Variable %in% c("fellow_baseline_VA", "fellow_intermediate_VA", "n_eyes")) |>
  pull(Variable)
```
...
log_message(paste0("Post-imputation fellow eye correlations - Baseline: ",
                  min(fellow_cor_df$cor_baseline, na.rm = TRUE), " to ",
                  max(fellow_cor_df$cor_baseline, na.rm = TRUE),
                  ", Intermediate: ",
                  min(fellow_cor_df$cor_intermediate, na.rm = TRUE), " to ",
                  max(fellow_cor_df$cor_intermediate, na.rm = TRUE)))
...
if (any(!cor_warnings$baseline_ok | !cor_warnings$intermediate_ok, na.rm = TRUE)) {
  warning("Some imputed datasets have fellow eye correlations outside expected range (0.5-0.95)")
}
```
...
kable(fellow_cor_df |> select(-starts_with("n_")), 
      col.names = c("Dataset", "Baseline", "Intermediate"),
...
kable(fellow_cor_df |> select(dataset, starts_with("n_")), 
      col.names = c("Dataset", "N Baseline", "N Intermediate"),
