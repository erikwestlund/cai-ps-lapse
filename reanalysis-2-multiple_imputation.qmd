...
custom_methods["cohort_id"] <- ""  # never impute
...
# Never impute but can be used as predictors: ever_lapse_binary, cohort_id
...
## Imputation Diagnostics (selected variables)

```{r imputation-diagnostics}
# Show diagnostic plots only when a variable has missingness
if (sum(is.na(df_mice$CCI)) > 0) {
  densityplot(mice_results, ~ CCI, main = "CCI") |> print()
  stripplot(mice_results, CCI ~ .imp, pch = 20, cex = 1.0, xlab = "Dataset (1=Original, 2+=Imputed)", main = "CCI: Original vs Imputed") |> print()
}
if (sum(is.na(df_mice$DCSI)) > 0) {
  densityplot(mice_results, ~ DCSI, main = "DCSI") |> print()
  stripplot(mice_results, DCSI ~ .imp, pch = 20, cex = 1.0, xlab = "Dataset (1=Original, 2+=Imputed)", main = "DCSI: Original vs Imputed") |> print()
}
if (sum(is.na(df_mice$baseline_VA_logMAR)) > 0) {
  densityplot(mice_results, ~ baseline_VA_logMAR, main = "Baseline VA (logMAR)") |> print()
  stripplot(mice_results, baseline_VA_logMAR ~ .imp, pch = 20, cex = 1.0, xlab = "Dataset (1=Original, 2+=Imputed)", main = "Baseline VA: Original vs Imputed") |> print()
}
if (sum(is.na(df_mice$intermediate_VA_logMAR)) > 0) {
  densityplot(mice_results, ~ intermediate_VA_logMAR, main = "Intermediate VA (logMAR)") |> print()
  stripplot(mice_results, intermediate_VA_logMAR ~ .imp, pch = 20, cex = 1.0, xlab = "Dataset (1=Original, 2+=Imputed)", main = "Intermediate VA: Original vs Imputed") |> print()
}
if (sum(is.na(df_mice$outcome_VA_logMAR)) > 0) {
  densityplot(mice_results, ~ outcome_VA_logMAR, main = "Outcome VA at 2 Years (logMAR)") |> print()
  stripplot(mice_results, outcome_VA_logMAR ~ .imp, pch = 20, cex = 1.0, xlab = "Dataset (1=Original, 2+=Imputed)", main = "Outcome VA: Original vs Imputed") |> print()
}

# person_dr (ordinal categorical): summarize proportions across imputations
if (sum(is.na(df_mice$person_dr)) > 0) {
  ...
  if (!is.null(dim(dr_props))) {
    dr_summary <- data.frame(
      Category = rownames(dr_props),
      Mean_Proportion = apply(dr_props, 1, mean) |> round(3),
      SD = apply(dr_props, 1, sd) |> round(3)
    )
    kable(dr_summary, caption = "DR severity proportions across imputations")
  }
}

# Categorical example: cataract surgery proportions across imputations
...
# Add back variables not included in imputation or transformations (exact row match on row_id)
