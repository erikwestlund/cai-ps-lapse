...
```{r setup-predictors}
# Create predictor matrix based on theoretical relationships
...
# Set default predictors for imputation variables
# Demographics predict most variables
demographic_vars <- c("age", "gender_cat", "race_ethnic_cat", "insurance_cat")
for (var in c(vars_impute_continuous, vars_impute_binary, vars_impute_categorical, vars_impute_ordinal)) {
  if (var %in% rownames(pred_matrix)) {
    for (demo in demographic_vars) {
      if (demo %in% colnames(pred_matrix)) {
        pred_matrix[var, demo] <- 1
      }
    }
  }
}

# Comorbidities predict most variables
comorbidity_vars <- c("CCI", "DCSI", "glaucoma_bef_hitplus_cat", "otherretina_bef_hitplus_cat")
for (var in c(vars_impute_continuous, vars_impute_binary, vars_impute_categorical, vars_impute_ordinal)) {
  if (var %in% rownames(pred_matrix)) {
    for (comorb in comorbidity_vars) {
      if (comorb %in% colnames(pred_matrix)) {
        pred_matrix[var, comorb] <- 1
      }
    }
  }
}

# Treatment variables predict outcomes and DR severity
treatment_vars <- c("anti_VEGF", "focal_laser_flag", "PRP_flag", "other_inject")
for (var in c("intermediate_VA_logMAR", "outcome_VA_logMAR", "person_dr")) {
  if (var %in% rownames(pred_matrix)) {
    for (tr in treatment_vars) {
      if (tr %in% colnames(pred_matrix)) {
        pred_matrix[var, tr] <- 1
      }
    }
  }
}

# Fellow-eye variables predict corresponding variables
fellow_vars <- c("fellow_baseline_VA", "fellow_intermediate_VA", "fellow_outcome_VA", "fellow_dr")
if ("baseline_VA_logMAR" %in% rownames(pred_matrix) && "fellow_baseline_VA" %in% colnames(pred_matrix)) {
  pred_matrix["baseline_VA_logMAR", "fellow_baseline_VA"] <- 1
}
if ("intermediate_VA_logMAR" %in% rownames(pred_matrix) && "fellow_intermediate_VA" %in% colnames(pred_matrix)) {
  pred_matrix["intermediate_VA_logMAR", "fellow_intermediate_VA"] <- 1
}
if ("outcome_VA_logMAR" %in% rownames(pred_matrix) && "fellow_outcome_VA" %in% colnames(pred_matrix)) {
  pred_matrix["outcome_VA_logMAR", "fellow_outcome_VA"] <- 1
}
if ("person_dr" %in% rownames(pred_matrix) && "fellow_dr" %in% colnames(pred_matrix)) {
  pred_matrix["person_dr", "fellow_dr"] <- 1
}

# Exposure variable predicts outcomes
if ("ever_lapse_binary" %in% colnames(pred_matrix)) {
  for (var in c("baseline_VA_logMAR", "intermediate_VA_logMAR", "outcome_VA_logMAR", "person_dr")) {
    if (var %in% rownames(pred_matrix)) {
      pred_matrix[var, "ever_lapse_binary"] <- 1
    }
  }
}

# Temporal relationships for VA variables
if ("intermediate_VA_logMAR" %in% rownames(pred_matrix) && "baseline_VA_logMAR" %in% colnames(pred_matrix)) {
  pred_matrix["intermediate_VA_logMAR", "baseline_VA_logMAR"] <- 1
}
if ("outcome_VA_logMAR" %in% rownames(pred_matrix) && "baseline_VA_logMAR" %in% colnames(pred_matrix)) {
  pred_matrix["outcome_VA_logMAR", "baseline_VA_logMAR"] <- 1
}
if ("outcome_VA_logMAR" %in% rownames(pred_matrix) && "intermediate_VA_logMAR" %in% colnames(pred_matrix)) {
...
# Add theoretically important relationships even if correlation is low
# Demographics should predict each other
demographic_vars <- c("age", "gender_cat", "race_ethnic_cat", "insurance_cat")
...
# Comorbidities should predict DR severity and vice versa
if ("person_dr" %in% rownames(pred_matrix)) {
  if ("CCI" %in% colnames(pred_matrix)) pred_matrix["person_dr", "CCI"] <- 1
  if ("DCSI" %in% colnames(pred_matrix)) pred_matrix["person_dr", "DCSI"] <- 1
  if ("glaucoma_bef_hitplus_cat" %in% colnames(pred_matrix)) pred_matrix["person_dr", "glaucoma_bef_hitplus_cat"] <- 1
  if ("otherretina_bef_hitplus_cat" %in% colnames(pred_matrix)) pred_matrix["person_dr", "otherretina_bef_hitplus_cat"] <- 1
  if ("catsurg_before_hitplus_cat" %in% colnames(pred_matrix)) pred_matrix["person_dr", "catsurg_before_hitplus_cat"] <- 1
  if ("fellow_dr" %in% colnames(pred_matrix)) pred_matrix["person_dr", "fellow_dr"] <- 1
}

# Treatment variables should predict each other (treatment patterns)
treatment_vars <- c("anti_VEGF", "focal_laser_flag", "PRP_flag", "other_inject")
...
# Ensure demographics predict VA timepoints and DR severity
va_vars <- c("baseline_VA_logMAR", "intermediate_VA_logMAR", "outcome_VA_logMAR", "person_dr")
for (va in va_vars) {
  for (demo in demographic_vars) {
    if (va %in% rownames(pred_matrix) && demo %in% colnames(pred_matrix)) {
      pred_matrix[va, demo] <- 1
    }
  }
}

# Ensure ocular comorbidities and cataract surgery predict VA and DR severity
ocular_vars <- c("glaucoma_bef_hitplus_cat", "otherretina_bef_hitplus_cat", "catsurg_before_hitplus_cat")
for (va in va_vars) {
  for (ov in ocular_vars) {
    if (va %in% rownames(pred_matrix) && ov %in% colnames(pred_matrix)) {
      pred_matrix[va, ov] <- 1
    }
  }
}

# Ensure treatments predict intermediate and outcome VA
for (va in c("intermediate_VA_logMAR", "outcome_VA_logMAR")) {
  for (tr in treatment_vars) {
    if (va %in% rownames(pred_matrix) && tr %in% colnames(pred_matrix)) {
      pred_matrix[va, tr] <- 1
    }
  }
}

# Fellow-eye predictors: allow them to predict VA and DR
fellow_vars <- c("fellow_baseline_VA", "fellow_intermediate_VA", "fellow_outcome_VA", "fellow_dr")
...
if ("fellow_intermediate_VA" %in% colnames(pred_matrix)) pred_matrix["intermediate_VA_logMAR", "fellow_intermediate_VA"] <- 1
if ("fellow_outcome_VA" %in% colnames(pred_matrix)) pred_matrix["outcome_VA_logMAR", "fellow_outcome_VA"] <- 1
if ("fellow_dr" %in% colnames(pred_matrix) && "person_dr" %in% rownames(pred_matrix)) pred_matrix["person_dr", "fellow_dr"] <- 1

# Fellow-eye VA (baseline/intermediate) also inform DR severity
if ("fellow_baseline_VA" %in% colnames(pred_matrix) && "person_dr" %in% rownames(pred_matrix)) pred_matrix["person_dr", "fellow_baseline_VA"] <- 1
if ("fellow_intermediate_VA" %in% colnames(pred_matrix) && "person_dr" %in% rownames(pred_matrix)) pred_matrix["person_dr", "fellow_intermediate_VA"] <- 1

# Ocular comorbidities and cataract surgery predict VA timepoints
for (va in c("baseline_VA_logMAR", "intermediate_VA_logMAR", "outcome_VA_logMAR")) {
  for (ov in c("glaucoma_bef_hitplus_cat", "otherretina_bef_hitplus_cat", "catsurg_before_hitplus_cat")) {
    if (va %in% rownames(pred_matrix) && ov %in% colnames(pred_matrix)) pred_matrix[va, ov] <- 1
  }
}

# Exposure predicts VA timepoints (lapse history)
for (va in c("baseline_VA_logMAR", "intermediate_VA_logMAR", "outcome_VA_logMAR")) {
  if (va %in% rownames(pred_matrix) && "ever_lapse_binary" %in% colnames(pred_matrix)) pred_matrix[va, "ever_lapse_binary"] <- 1
}

log_message("Predictor matrix configured with temporal constraints")

# Print summary of predictor counts for each variable
pred_counts <- data.frame(
  Variable = rownames(pred_matrix),
  N_Predictors = rowSums(pred_matrix > 0),
  N_Predicting = colSums(pred_matrix > 0)[rownames(pred_matrix)]
)
pred_counts <- pred_counts[order(-pred_counts$N_Predictors), ]

log_message("\nTop variables by number of predictors:")
print(head(pred_counts, 15))
```
