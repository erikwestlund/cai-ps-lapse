...
method_table |>
  kable(caption = "Default imputation methods by variable")
...
# Don't impute these (already complete)
custom_methods["e_mrn_deidentified"] <- ""
custom_methods["ever_lapse_binary"] <- ""
custom_methods["glaucoma_bef_hitplus_cat"] <- ""  # Already recoded
custom_methods["otherretina_bef_hitplus_cat"] <- ""  # Already recoded
...
# Number of imputations (typically 20-40 for small amounts of missingness)
n_imputations <- 30

# Run mice imputation
# Note: This may take several minutes depending on dataset size
message("Starting multiple imputation with ", n_imputations, " imputations...")
...
# Person DR - compare observed vs imputed
if(sum(is.na(df_mice$person_dr)) > 0) {
  # Check distribution of imputed values
  prop_table_dr <- with(mice_results, table(person_dr)) |> 
    lapply(prop.table) |> 
    do.call(rbind, args = _)
  
  message("Proportion of DR severity (0=No DR, 1=NPDR, 2=PDR) across imputations:")
  print(summary(prop_table_dr))
}

# Check imputation quality for categorical variables
# Cataract surgery
if(sum(is.na(df_mice$catsurg_before_hitplus_cat)) > 0) {
  prop_table <- with(mice_results, table(catsurg_before_hitplus_cat)) |> 
    lapply(prop.table) |> 
    do.call(rbind, args = _)
  
  message("Proportion of cataract surgery across imputations:")
  print(summary(prop_table))
}
```
