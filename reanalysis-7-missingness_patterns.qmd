...
for (var in available_vars) {
  if (var %in% names(original_data)) {
    if (var %in% factor_vars && is.factor(original_data[[var]])) {
      # For factor variables, calculate SMD for each level
      ...
      if (sum(!is.na(original_data[[var]])) > 0) {
        ...
        sd_pooled <- sqrt((var(original_data[[var]][original_data$excluded], na.rm = TRUE) + 
                          var(original_data[[var]][!original_data$excluded], na.rm = TRUE)) / 2)
