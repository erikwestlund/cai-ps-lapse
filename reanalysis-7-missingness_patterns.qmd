...
# 2. Load new data with cohort assignments (from reanalysis-1)
cohort_file <- file.path(reanalysis_data_dir, "df_with_cohort.rds")
if (!file.exists(cohort_file)) {
  stop("Cohort data not found. Run reanalysis-1-data_preparation.qmd first.")
}
cohort_data <- readRDS(cohort_file)

# Split into submitted and excluded
submitted_data <- cohort_data[cohort_data$cohort_id == "submitted", ]
excluded_data <- cohort_data[cohort_data$cohort_id == "excluded", ]
