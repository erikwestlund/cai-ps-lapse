...
smd_raw <- ExtractSmd(table1)

# ExtractSmd returns a list - extract just the numeric SMD values
# The list has one element per variable, plus a "Variable" element with names
if (is.list(smd_raw) && "Variable" %in% names(smd_raw)) {
  # Get variable names from the Variable element
  var_names <- smd_raw$Variable
  # Get SMD values (all elements except Variable)
  smd_values <- unlist(smd_raw[names(smd_raw) != "Variable"])
...
} else if (is.data.frame(smd_raw)) {
  # If it's a data frame, use rownames as variable names
  smd_data <- data.frame(
    Variable = rownames(smd_raw),
    SMD = smd_raw[, 1],
    stringsAsFactors = FALSE
  )
} else {
  # Fallback: use available_vars as variable names
  smd_values <- unlist(smd_raw)
  smd_values <- smd_values[!is.na(suppressWarnings(as.numeric(smd_values)))]
  
  smd_data <- data.frame(
    Variable = available_vars[1:length(smd_values)],
...
# Plot 1: SMD plot
p1 <- ggplot(smd_data, aes(x = reorder(Variable, abs(SMD)), y = SMD)) +
  geom_point(size = 3, color = ifelse(abs(smd_data$SMD) > 0.1, "red", "blue")) +
