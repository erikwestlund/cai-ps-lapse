...
```{r identify-groups}
# Check if cohort_id exists, if not use a unique identifier
id_var <- if("cohort_id" %in% names(original_data)) "cohort_id" else "e_mrn_deidentified"

# Alternative approach: identify excluded based on missing outcome
# Patients were excluded from imputation if they had missing outcomes
if ("outcome_va_vi_binary" %in% names(original_data)) {
  ...
  # Fall back to comparing with imputed dataset IDs
  # Get the IDs from the first imputed dataset (should be same across all)
  included_ids <- unique(imputed_datasets[[1]][[id_var]])
  
  # Get all IDs from original data
  all_ids <- unique(original_data[[id_var]])
  
  # Identify excluded patients (those in original but not in imputed)
  excluded_ids <- setdiff(all_ids, included_ids)
  
  # Create indicator in original data (TRUE = excluded, FALSE = included)
  original_data$excluded <- original_data[[id_var]] %in% excluded_ids
  log_message(paste("Using ID comparison to identify excluded patients.",
                   "Original IDs:", length(all_ids),
                   "Included IDs:", length(included_ids)))
}
...
# Based on the actual column names used in functions.R formulas
# Core demographic variables
demographic_vars <- c("age", "age_cat", "gender_cat", "race_ethnic_cat")
...
all_vars <- c(demographic_vars, insurance_vars, diabetes_vars, 
              comorbidity_vars, vision_vars, exposure_var, outcome_vars)
...
# Specify variable types (using actual column names from formulas)
factor_vars <- c("age_cat", "gender_cat", "race_ethnic_cat", "insurance_cat", 
                 "insulin", "HTN", "dyslipidemia", "CVD",
