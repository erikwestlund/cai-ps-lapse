---
title: "Reanalysis Step 6: Sensitivity Analysis with E-values"
...
# Initialize logging
log_file <- init_log("sensitivity_analysis_interaction")

# Load outcome analysis results from interaction analysis
outcome_results <- readRDS(file.path(reanalysis_data_dir, "outcome_analysis_results.rds"))

# Extract the saved components
# pooled_coefficients is a list with odds_ratios, risk_differences, emmeans_list
pooled_results <- outcome_results$pooled_coefficients
subgroup_effects <- outcome_results$subgroup_effects  # This has the OR effects by subgroup
risk_differences <- outcome_results$risk_differences   # Risk differences on probability scale
```
...
```{r overall-evalue}
# For interaction analysis, there's no single main effect
# Instead, we'll calculate an overall average effect across all subgroups
# This is a weighted average of the subgroup-specific effects

# Get the odds ratios from the subgroup effects
all_log_odds <- log(subgroup_effects$OR)
all_se <- subgroup_effects$SE  # SE is on log odds scale

# Calculate overall average effect (unweighted for simplicity)
# In practice, might want to weight by subgroup size
avg_log_odds <- mean(all_log_odds, na.rm = TRUE)
avg_se <- sqrt(mean(all_se^2, na.rm = TRUE))  # Root mean square of SEs

# Convert to OR scale
or_main <- exp(avg_log_odds)
ci_lower_main <- exp(avg_log_odds - 1.96 * avg_se)
ci_upper_main <- exp(avg_log_odds + 1.96 * avg_se)

```

**Average Effect of Lapsing Across All Subgroups:**  
...
evalue_summary_main <- data.frame(
  Effect = "Average Effect Across Subgroups",
  OR = round(or_main, 3),
...
for (i in 1:nrow(subgroup_effects)) {
  ...
  # Get OR and CI (subgroup_effects already has OR, OR_CI_Lower, OR_CI_Upper)
  or_val <- subgroup$OR
  ci_lower <- subgroup$OR_CI_Lower
  ci_upper <- subgroup$OR_CI_Upper
...
# For interaction analysis, we don't have main effect coefficients for confounders
# Instead, we'll note that benchmarking is not directly applicable
# since the interaction model focuses on subgroup-specific effects
benchmark_effects <- data.frame()
...
**Note:** Benchmarking against measured confounders is not directly applicable in this interaction analysis, as we focus on subgroup-specific treatment effects rather than main effects of confounders.

**Minimum E-value across subgroups:** `r round(min_evalue, 2)`

**Interpretation:** `r if(min_evalue > 2) "The minimum E-value suggests good robustness to unmeasured confounding." else if(min_evalue > 1.5) "The minimum E-value suggests moderate robustness to unmeasured confounding." else "Some subgroups may be sensitive to unmeasured confounding."`
...
#| include: false
# Finalize logging
log_message("Sensitivity analysis with E-values completed successfully")
