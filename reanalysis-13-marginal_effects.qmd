...
for (sg_name in names(subgroups)) {
  ...
  if (is.null(sg$subset_condition)) {
    ...
    mfx <- margins(model, design = design, variables = "ever_lapse")
  ...
  if (!is.null(test_results[[sg_name]])) {
    ...
    if ("ever_lapse" %in% rownames(mfx_summary)) {
      ame <- mfx_summary["ever_lapse", "AME"]
      se <- mfx_summary["ever_lapse", "SE"]
...
for (i in 1:n_imputations) {
  ...
  tryCatch({
    ...
    for (sg_name in names(subgroups)) {
      ...
      # Store results
      if (!is.null(mfx_summary) && "ever_lapse" %in% rownames(mfx_summary)) {
