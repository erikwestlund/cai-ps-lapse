...
# Create indicator for which outcomes were imputed
...
imputation_summary <- df_original |>
  ...
  summarise(
    ...
    had_intermediate = sum(had_intermediate),
    .groups = "drop"
  )

kable(imputation_summary,
      col.names = c("Outcome Imputed", "N", "%", "Has Baseline", "Has Intermediate"),
      caption = "Summary of imputation status for outcome VA")
```
...
```{r summary-stats}
# Compare change scores between imputed and observed
...
kable(change_summary |> 
        select(group, n, mean_change, sd_change, median_change, iqr_change, min_change, max_change),
      digits = 3,
      col.names = c("Group", "N", "Mean", "SD", "Median", "IQR", "Min", "Max"),
      caption = "Distribution of vision changes (outcome - baseline) in logMAR units")
...
test_results <- data.frame(
  Test = c("Welch's t-test", "Kolmogorov-Smirnov"),
  Statistic = c(t_test$statistic, ks_test$statistic),
  P_Value = c(t_test$p.value, ks_test$p.value),
  Interpretation = c(
    ifelse(t_test$p.value < 0.05, "Means differ significantly", "Means similar"),
    ifelse(ks_test$p.value < 0.05, "Distributions differ significantly", "Distributions similar")
  )
)

kable(test_results, digits = 4, caption = "Statistical tests comparing change distributions")
```
...
p1 <- ggplot(plot_data, aes(x = change_baseline_to_outcome, fill = Group)) +
  ...
  labs(
    title = "Distribution of Vision Changes",
...
# 2. Boxplots
p2 <- ggplot(plot_data, aes(x = Group, y = change_baseline_to_outcome, fill = Group)) +
  geom_boxplot(alpha = 0.7, outlier.alpha = 0.3) +
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  scale_fill_manual(values = c("Observed" = "#2E86AB", "Imputed" = "#A23B72")) +
  labs(
    title = "Vision Changes by Group",
...
# Compare proportions of improvement/stable/worsening
