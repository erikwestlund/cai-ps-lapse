...
baseline_chars <- df_original |>
  ...
  summarise(
    ...
    mean_DCSI = mean(DCSI, na.rm = TRUE),
    sd_DCSI = sd(DCSI, na.rm = TRUE),
...
# Merge baseline characteristics with change scores using row_id (already added earlier)
analysis_df <- changes_df |>
  left_join(df_original |> select(row_id, CCI, DCSI, age), 
            by = "row_id")
