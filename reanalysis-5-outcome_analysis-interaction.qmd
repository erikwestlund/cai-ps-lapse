...
crosstab_wide <- crosstab_df |>
  pivot_wider(names_from = Lapse_Status, 
              values_from = Count,
              values_fill = 0,
              names_prefix = "Lapse_") |>  # Add prefix to avoid numeric column names
  arrange(DR_Severity, Treatment) |>
  mutate(
    Total = Lapse_0 + Lapse_1,
    No_Lapse_Pct = paste0(round(100 * Lapse_0 / Total, 1), "%"),
    Lapsed_Pct = paste0(round(100 * Lapse_1 / Total, 1), "%"),
    Row_Pct = paste0(round(100 * Total / total_n, 1), "%")
  ) |>
  select(DR_Severity, Treatment, 
         No_Lapse = Lapse_0, No_Lapse_Pct,
         Lapsed = Lapse_1, Lapsed_Pct,
         Total, Row_Pct)
