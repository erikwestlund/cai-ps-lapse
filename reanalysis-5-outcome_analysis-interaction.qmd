...
if (file.exists(models_cache_file) && !force_refit_models) {
  # Load cached models structure
  cached_data <- readRDS(models_cache_file)
  
  # Extract components from the cached structure
  models <- cached_data$models
  model_progress <- cached_data$model_progress
  
  # Extract or initialize model_summaries
  if ("model_summaries" %in% names(cached_data)) {
    model_summaries <- cached_data$model_summaries
  } else {
    # Rebuild model_summaries from models if not in cache
    model_summaries <- list()
    for (i in 1:length(models)) {
      model_summaries[[i]] <- summary(models[[i]])$coefficients
    }
  }
  
  models_loaded_from_cache <- TRUE
  cache_location <- models_cache_file
  log_message(paste0("Models loaded from cache: ", cache_location))
} else {
  # Fit new models
  ...
  # Save to cache with full structure
  cached_data <- list(
    models = models,
  ...
  saveRDS(cached_data, models_cache_file)
  cache_location <- models_cache_file
