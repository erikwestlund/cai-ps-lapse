...
if (file.exists(models_cache_file) && !force_refit_models) {
  # Load cached models (just the models list, not a complex structure)
  models <- readRDS(models_cache_file)
  models_loaded_from_cache <- TRUE
  cache_location <- models_cache_file
  log_message("Models loaded from cache: ", cache_location)
  
  # Rebuild model_progress from loaded models
  ...
  # Save to cache - models only, not the full cached_models list to save memory
  saveRDS(models, models_cache_file)
  cache_location <- models_cache_file
