...
library(marginaleffects)  # For proper AME calculation with correct SEs
...
force_recalc_ame <- FALSE  # Set to TRUE to recalculate AMEs with marginaleffects
...
pool_emmeans_subgroup_effects <- function(models, n_imputations) {
  ...
  pooled_or_results <- data.frame(
    ...
    log_odds = numeric(n_subgroups),
    SE_log_odds = numeric(n_subgroups),
    df = numeric(n_subgroups),
    stringsAsFactors = FALSE
  )
  ...
  pooled_rd_results <- data.frame(
    ...
    risk_diff = numeric(n_subgroups),
    SE_risk_diff = numeric(n_subgroups),
    df = numeric(n_subgroups),
    stringsAsFactors = FALSE
  )
  ...
  for (j in 1:n_subgroups) {
    ...
    log_odds_estimates <- sapply(all_contrasts, function(x) x$estimate[j])
    log_odds_SEs <- sapply(all_contrasts, function(x) x$SE[j])
    
    # Apply Rubin's rules for log odds
    Q_bar_log <- mean(log_odds_estimates)
    U_bar_log <- mean(log_odds_SEs^2)
    B_log <- var(log_odds_estimates)
    ...
    rd_estimates <- sapply(all_risk_diffs, function(x) x$estimate[j])
    rd_SEs <- sapply(all_risk_diffs, function(x) x$SE[j])
    
    # Apply Rubin's rules for risk differences
    Q_bar_rd <- mean(rd_estimates)
    U_bar_rd <- mean(rd_SEs^2)
    B_rd <- var(rd_estimates)
...
```{r ame-calculation}
# Function to calculate and pool AMEs across imputations using marginaleffects
calculate_pooled_ames <- function(imputed_datasets, models, n_imputations) {
  ...
  message("Using marginaleffects for proper standard error calculation")
  ...
  for (imp in 1:n_imputations) {
    ...
    for (dr_level in c("No_DR", "NPDR", "PDR")) {
      for (tx_status in c("No_Treatment", "Any_Treatment")) {
        ...
        subset_indices <- which(data_for_ame$dr_severity == dr_level & 
                               data_for_ame$any_treatment == tx_status)
        n_subset <- length(subset_indices)
        
        if (n_subset > 0) {
          subset_data <- data_for_ame[subset_indices, ]
          
          tryCatch({
            # Calculate average marginal effect using marginaleffects
            # This properly accounts for model uncertainty and survey design
            ame_result <- avg_comparisons(
              model_for_ame,
              variables = "ever_lapse_binary",
              comparison = "difference",  # Difference in probabilities
              newdata = subset_data,
              wts = "iptw_weight",  # Use the survey weights column
              vcov = TRUE  # Use the model's vcov (accounts for clustering)
            )
            
            # Extract the AME and SE
            ame_value <- ame_result$estimate[1]
            se_value <- ame_result$std.error[1]
            
          }, error = function(e) {
            # Fallback to manual calculation with warning
            message("    Warning: marginaleffects failed for ", dr_level, " + ", tx_status)
            message("    Error: ", e$message)
            message("    Using manual calculation (SEs may be underestimated)")
            ...
            # Note: This SE is likely underestimated
            weighted_var <- sum(weights * (individual_effects - ame_value)^2) / sum(weights)
            se_value <- sqrt(weighted_var / n_subset) * 2  # Multiply by 2 as crude adjustment
          })
  ...
  # Pool AME results across imputations using Rubin's rules
  ...
  for (j in 1:n_subgroups) {
    ...
    T <- U_bar + (1 + 1/n_imputations) * B  # Total variance
    
    # Degrees of freedom (Barnard-Rubin)
    lambda <- (B + B/n_imputations) / T
    df_old <- (n_imputations - 1) / (lambda^2)
    
    pooled_ame$AME[j] <- Q_bar
    pooled_ame$SE[j] <- sqrt(T)
    pooled_ame$df[j] <- df_old
  }
  
  # Add confidence intervals and p-values using t-distribution with proper df
  pooled_ame <- pooled_ame |>
    mutate(
      CI_Lower = AME - qt(0.975, df) * SE,
      CI_Upper = AME + qt(0.975, df) * SE,
      t_stat = AME / SE,
...
# Check if AME results are cached
ame_cache_file <- file.path(reanalysis_data_dir, "ame_results_interaction.rds")

if (file.exists(ame_cache_file) && !force_recalc_ame) {
  # Load cached results
  ame_results <- readRDS(ame_cache_file)
  ame_loaded_from_cache <- TRUE
  ame_cache_location <- ame_cache_file
  message("AME results loaded from cache: ", ame_cache_location)
} else {
  # Calculate new AME results
  ...
  # Save to cache
  saveRDS(ame_results, ame_cache_file)
  ame_cache_location <- ame_cache_file
  message("AME results calculated and saved to cache: ", ame_cache_location)
}
...
### Forest Plot of Average Marginal Effects
...
forest_plot <- ggplot(forest_data, aes(x = AME, y = Subgroup)) +
  ...
  # Labels and theme
  labs(
    title = "Average Marginal Effects of Lapsing on Vision Impairment",
    subtitle = "By DR severity and treatment status (with 95% CI)",
  ...
  scale_shape_manual(values = c(
    "No_Treatment" = 16,  # Circle
    "Any_Treatment" = 17   # Triangle
  )) +
...
forest_plot_annotated <- forest_plot +
  geom_text(aes(label = sprintf("%.3f", AME)),
            nudge_y = 0.3, size = 3.5, color = "black")
