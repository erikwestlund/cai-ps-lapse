  ...
  for (j in 1:n_subgroups) {
    ...
    log_odds_estimates <- sapply(all_contrasts, function(x) {
      if ("estimate" %in% names(x)) {
        ...
        x[j, which(names(x) %in% c("estimate", "emmean", "contrast"))[1]]
      }
    })
    ...
    log_odds_SEs <- sapply(all_contrasts, function(x) {
      if ("SE" %in% names(x)) {
        x$SE[j]
      } else if ("std.error" %in% names(x)) {
        x$std.error[j]
      } else {
        x[j, which(names(x) %in% c("SE", "std.error", "se"))[1]]
      }
    })
    ...
    # Check for column names - emmeans may use 'estimate' or other names
    rd_estimates <- sapply(all_risk_diffs, function(x) {
      if ("estimate" %in% names(x)) {
        x$estimate[j]
      } else if ("emmean" %in% names(x)) {
        x$emmean[j]
      } else if ("prob" %in% names(x)) {
        x$prob[j]
      } else {
        # Try to get the estimate column (might be named differently)
        x[j, which(names(x) %in% c("estimate", "emmean", "prob", "prediction"))[1]]
      }
    })
