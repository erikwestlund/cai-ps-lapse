...
pooled_ame <- data.frame(
  ...
  Interpretation = paste0(
    ...
    round(mean(ame_combined$ame) * 100, 1), " percentage points"
  )
)
...
- **Effect size**: The average marginal effect indicates that lapsing increases the probability of VI/blindness by approximately `r round(mean(ame_combined$ame) * 100, 1)` percentage points
- **Consistency**: The effect is present across all levels of DR severity and treatment status
- **Key insight**: Even patients with no DR and no treatment show increased risk when lapsing from care

This simpler model provides clearer interpretation than the interaction model while maintaining the core finding about the importance of continuous care.