...
# Create clean results table using the pooled contrasts
subgroup_effects <- data.frame(
  DR_Severity = pooled_lapse_effects$dr_severity,
  Treatment = pooled_lapse_effects$any_treatment,
  Log_Odds = round(pooled_lapse_effects$estimate, 4),
  SE = round(pooled_lapse_effects$SE, 4),
  CI_Lower = round(pooled_lapse_effects$CI_Lower, 4),
  CI_Upper = round(pooled_lapse_effects$CI_Upper, 4),
  OR = round(exp(pooled_lapse_effects$estimate), 4),
  P_Value = format.pval(pooled_lapse_effects$p.value, digits = 4, eps = 0.0001)
)
...
manual_effects <- manual_effects |>
  ...
  mutate(P_Value = format.pval(P_Value, digits = 4, eps = 0.0001)) |>
  select(-Z_stat)
...
for (dr_level in c("No_DR", "NPDR", "PDR")) {
  for (tx_status in c("No_Treatment", "Any_Treatment")) {
    ...
    if (n_subset > 0) {
      ...
      # Bootstrap SE would be ideal here; using simple weighted SE for now
      weighted_var <- sum(weights * (individual_effects - ame_value)^2) / sum(weights)
      se_value <- sqrt(weighted_var / n_subset)
...
```{r predicted-probs}
# Function to pool predicted probabilities across imputations
pool_predicted_probabilities <- function(models, n_imputations) {
  
  message("\nCalculating pooled predicted probabilities...")
  
  # Store probabilities from each imputation
  all_probs <- list()
  
  for (i in 1:n_imputations) {
    emm_probs <- emmeans(models[[i]], 
                         ~ ever_lapse_binary | dr_severity * any_treatment,
                         type = "response",
                         weights = "proportional")
    df_probs <- as.data.frame(emm_probs)
    
    # Debug: print column names for first iteration
    if (i == 1) {
      message("emmeans response columns: ", paste(names(df_probs), collapse = ", "))
    }
    
    # The response scale should have columns like: prob, asymp.LCL, asymp.UCL, SE
    # Or it might be: response, asymp.LCL, asymp.UCL, SE
    # Find the main estimate column
    prob_col <- if ("prob" %in% names(df_probs)) {
      "prob"
    } else if ("response" %in% names(df_probs)) {
      "response"
    } else if ("emmean" %in% names(df_probs)) {
      "emmean"
    } else {
      # Just use the first numeric column that's not a CI
      numeric_cols <- names(df_probs)[sapply(df_probs, is.numeric)]
      numeric_cols[!grepl("CL|CI|df|SE", numeric_cols)][1]
    }
    
    # Find SE column
    se_col <- if ("SE" %in% names(df_probs)) {
      "SE"
    } else if ("se" %in% names(df_probs)) {
      "se"
    } else {
      # Calculate from confidence intervals if available
      NULL
    }
    
    # Standardize column names
    df_probs$prob <- if (!is.null(prob_col)) df_probs[[prob_col]] else NA
    df_probs$SE <- if (!is.null(se_col)) {
      df_probs[[se_col]]
    } else if (all(c("asymp.LCL", "asymp.UCL") %in% names(df_probs))) {
      # Approximate SE from CI width
      (df_probs$asymp.UCL - df_probs$asymp.LCL) / (2 * 1.96)
    } else {
      0.1  # Default SE if we can't calculate it
    }
    
    all_probs[[i]] <- df_probs
  }
  
  # Pool probabilities
  first_df <- all_probs[[1]]
  n_cells <- nrow(first_df)
  
  pooled_probs <- data.frame(
    ever_lapse_binary = first_df$ever_lapse_binary,
    dr_severity = first_df$dr_severity,
    any_treatment = first_df$any_treatment,
    prob = numeric(n_cells),
    SE = numeric(n_cells),
    stringsAsFactors = FALSE
  )
  
  for (j in 1:n_cells) {
    # Extract probabilities for this cell - now we know the column exists
    probs <- sapply(all_probs, function(x) x$prob[j])
    ses <- sapply(all_probs, function(x) x$SE[j])
    
    # Note: For probabilities, we might want to use logit transformation
    # for better pooling, but for simplicity using direct pooling
    pooled_probs$prob[j] <- mean(probs)
    
    # Pool standard errors
    U_bar <- mean(ses^2)
    B <- var(probs)
    T <- U_bar + (1 + 1/n_imputations) * B
    pooled_probs$SE[j] <- sqrt(T)
  }
  
  # Add confidence intervals
  pooled_probs <- pooled_probs |>
    mutate(
      CI_Lower = pmax(0, prob - 1.96 * SE),
      CI_Upper = pmin(1, prob + 1.96 * SE)
    )
  
  return(pooled_probs)
}

# Get pooled probabilities
prob_summary <- pool_predicted_probabilities(models, n_imputations)

# Reshape for presentation
prob_table <- prob_summary |>
  select(ever_lapse_binary, dr_severity, any_treatment, prob, CI_Lower, CI_Upper) |>
  mutate(
    Probability = paste0(round(prob, 3), " (", 
                        round(CI_Lower, 3), ", ", 
                        round(CI_Upper, 3), ")")
  ) |>
  select(dr_severity, any_treatment, ever_lapse_binary, Probability) |>
