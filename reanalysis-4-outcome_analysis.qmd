...
# Calculate derived statistics - check for NAs first
if (all(!is.na(manual_effects$Log_Odds)) && all(!is.na(manual_effects$SE))) {
...
# Create simple probability table - check if prob column exists first
if ("prob" %in% names(prob_df)) {
  prob_table <- prob_df |>
    mutate(
  ...
  # If prob doesn't exist, look for alternative column names
  prob_col_name <- names(prob_df)[grep("prob|response|emmean", names(prob_df), ignore.case = TRUE)[1]]
  if (!is.na(prob_col_name)) {
    prob_table <- prob_df |>
      mutate(
    ...
    # Create empty table if we can't find probability column
    prob_table <- data.frame(
      dr_severity = character(),
      any_treatment = character(),
      No_Lapse = numeric(),
      Lapsed = numeric()
    )
    message("Warning: Could not find probability column in emmeans output")
  }
}
