...
```{r define-formula}
# Load the standard formulas from functions.R
source("functions.R")

# Get the standard analysis formulas used in the original analysis
standard_formulas <- get_analysis_formulas()

# For reference, the original full formula is:
# standard_formulas$full

# For the reanalysis, we extend the full formula by adding:
# - dr_severity: diabetic retinopathy severity (No_DR, NPDR, PDR)
# - any_treatment: whether patient received any DR treatment
# These are new covariates not in the original analysis
main_effects_formula <- as.formula("outcome_va_vi_binary ~ 
  ever_lapse_binary + 
  dr_severity +
  any_treatment +
  baseline_VA_logMAR +
