...
pool_predicted_probabilities <- function(models, n_imputations) {
  ...
  for (i in 1:n_imputations) {
    ...
    # Find the probability column (could be "prob", "response", "emmean", etc.)
    prob_col <- names(df_probs)[grep("prob|response|emmean", names(df_probs), ignore.case = TRUE)[1]]
    se_col <- names(df_probs)[grep("SE|se", names(df_probs))[1]]
    
    # Standardize column names
    df_probs$prob <- df_probs[[prob_col]]
    df_probs$SE <- df_probs[[se_col]]
