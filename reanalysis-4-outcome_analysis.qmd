...
with(imputed_datasets[[1]], 
     ftable(dr_severity, any_treatment, ever_lapse_binary)) |> kable()
```
...
message("Ready to pool ", length(models), " models")
```

## Pool Results Using Rubin's Rules

```{r pool-results}
# Use mitools for robust pooling implementation
# MIcombine properly implements Rubin's rules with Barnard-Rubin adjustment

# Ensure models is a proper list for MIcombine
if (!is.list(models)) {
...
# Get pooled variance-covariance matrix for later use
pooled_vcov <- vcov(pooled_model)

# Check actual structure of pooled_summary
print(names(pooled_summary))
print(colnames(pooled_summary))

# Extract results based on actual MIcombine output structure
# MIcombine typically returns: results, se, (lower, upper), missInfo
pooled_results <- data.frame(
  Term = rownames(pooled_summary),
  Estimate = pooled_summary[, "results"],
  SE = pooled_summary[, "se"],
  CI_Lower = pooled_summary[, "(lower"],  
  CI_Upper = pooled_summary[, "upper)"],
  OR = exp(pooled_summary[, "results"]),
  OR_CI_Lower = exp(pooled_summary[, "(lower"]),
  OR_CI_Upper = exp(pooled_summary[, "upper)"])
)

# Calculate additional statistics if needed
pooled_results$t_stat <- pooled_results$Estimate / pooled_results$SE
pooled_results$P_Value <- 2 * pnorm(-abs(pooled_results$t_stat))
...
# Note: MIcombine doesn't provide df in the standard output
...
# Show key terms with their estimates and CIs
key_terms <- grep("ever_lapse", pooled_results$Term, value = TRUE)[1:min(3, length(grep("ever_lapse", pooled_results$Term, value = TRUE)))]
key_results <- pooled_results[pooled_results$Term %in% key_terms, c("Term", "Estimate", "SE", "P_Value")]
...
kable(key_results, caption = "Key lapse-related terms")
...
message("Performing pooled joint hypothesis tests...")
...
if (length(threeway_terms_test) > 0) {
  ...
  message("1. Three-way interaction test (H0: all three-way terms = 0):")
...
if (length(lapse_dr_terms) > 0) {
  ...
  message("2. Lapse × DR severity interaction test:")
...
if (length(lapse_tx_terms) > 0) {
  ...
  message("3. Lapse × treatment interaction test:")
...
pool_emmeans_contrasts <- function(models, n_imputations) {
  ...
  message("Calculating marginal means and contrasts across all imputations...")
...
message("=== OUTCOME ANALYSIS SUMMARY ===")
message("Three-Way Interaction Model Results:")
...
message("Pooling Quality Metrics:")
...
message("The three-way interaction reveals that:")
...
message("Implications for care continuity programs:")
