...
**Treatment Status:**
```{r treatment-dist}
table(imputed_datasets[[1]]$any_treatment) 
```
...
pooled_results <- data.frame(
  ...
  statistic = pooled_summary[, "(lower"],  
  df = pooled_summary[, "df"],
  p.value = pooled_summary[, "upper)"],
  CI_Lower = pooled_summary[, "2.5 %"],
  CI_Upper = pooled_summary[, "97.5 %"],
  OR = exp(pooled_summary[, "results"]),
  OR_CI_Lower = exp(pooled_summary[, "2.5 %"]),
  OR_CI_Upper = exp(pooled_summary[, "97.5 %"])
)

# Clean column names if needed
names(pooled_results) <- c("Term", "Estimate", "SE", "t_stat", "df", "P_Value", 
                           "CI_Lower", "CI_Upper", "OR", "OR_CI_Lower", "OR_CI_Upper")
...
key_df <- pooled_results[pooled_results$Term %in% key_terms, c("Term", "df")]
key_df$Term <- substr(key_df$Term, 1, 40)
key_df$df <- round(key_df$df, 1)

kable(key_df, caption = "Degrees of freedom for key terms")
