...
threeway_terms_test <- grep("ever_lapse.*:dr_severity.*:any_treatment", 
                            names(coef(model_for_tests)), value = TRUE)

if (length(threeway_terms_test) > 0) {
  hypothesis_3way <- paste0(threeway_terms_test, " = 0")
  
  joint_test_3way <- linearHypothesis(
    model_for_tests,
    hypothesis_3way,
    vcov = vcov(model_for_tests)
  )
  
  message("\n1. Three-way interaction test (H0: all three-way terms = 0):")
  message("   Chi-sq = ", round(joint_test_3way$Chisq[2], 3))
  message("   df = ", joint_test_3way$Df[2])
  message("   p-value = ", format.pval(joint_test_3way$`Pr(>Chisq)`[2], digits = 4, eps = 0.0001))
  
  if(joint_test_3way$`Pr(>Chisq)`[2] < 0.05) {
...
lapse_dr_terms <- grep("ever_lapse.*:dr_severity(?!.*:any_treatment)", 
                      names(coef(model_for_tests)), value = TRUE, perl = TRUE)

if (length(lapse_dr_terms) > 0) {
  hypothesis_lapse_dr <- paste0(lapse_dr_terms, " = 0")
  
  joint_test_lapse_dr <- linearHypothesis(
    model_for_tests,
    hypothesis_lapse_dr,
    vcov = vcov(model_for_tests)
  )
  ...
  message("   Chi-sq = ", round(joint_test_lapse_dr$Chisq[2], 3))
  message("   df = ", joint_test_lapse_dr$Df[2])
  message("   p-value = ", format.pval(joint_test_lapse_dr$`Pr(>Chisq)`[2], digits = 4, eps = 0.0001))
}
...
if (length(lapse_tx_terms) > 0) {
  ...
  joint_test_lapse_tx <- linearHypothesis(
    model_for_tests,
    hypothesis_lapse_tx,
    vcov = vcov(model_for_tests)
  )
  ...
  message("   Chi-sq = ", round(joint_test_lapse_tx$Chisq[2], 3))
  message("   df = ", joint_test_lapse_tx$Df[2])
  message("   p-value = ", format.pval(joint_test_lapse_tx$`Pr(>Chisq)`[2], digits = 4, eps = 0.0001))
}
```
...
pool_predicted_probabilities <- function(models, n_imputations) {
  ...
  for (i in 1:n_imputations) {
    emm_probs <- emmeans(models[[i]], 
                         ...
                         type = "response",
                         weights = "proportional")
    all_probs[[i]] <- as.data.frame(emm_probs)
  }
  ...
  for (j in 1:n_cells) {
    # Extract probabilities for this cell
...
prob_table <- prob_summary |>
  select(ever_lapse_binary, dr_severity, any_treatment, prob, asymp.LCL, asymp.UCL) |>
  mutate(
