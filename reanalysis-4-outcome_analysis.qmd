...
if (file.exists(models_cache_file) && !force_refit) {
  ...
  cat("Loaded cached models from:", models_cache_file, "\n")
  cat("To refit models, set force_refit <- TRUE\n")
} else {
  # Fit new models
  ...
  for (i in 1:n_imputations) {
    cat("Fitting model", i, "of", n_imputations, "\r")
  ...
  cat("Models cached to:", models_cache_file, "\n")
}
...
if (file.exists(ame_cache_file) && !force_recalc_ame) {
  ...
  cat("Loaded cached AMEs from:", ame_cache_file, "\n")
} else {
  # Calculate AME for each imputed dataset then pool
  cat("Calculating AMEs for each imputed dataset...\n")
  ame_list <- list()
  
  for (i in 1:n_imputations) {
    cat("Calculating AME", i, "of", n_imputations, "\r")
    imp_data <- imputed_datasets[[i]]
    imp_data$weights <- twang_weights[[i]]
  ...
  cat("AMEs cached to:", ame_cache_file, "\n")
}
...
```{r predicted-probs}
# Use emmeans for predicted probabilities - it handles survey models properly
# We'll use the first imputed model for illustration

# Get estimated marginal means for key combinations
emm_results <- tryCatch({
  # Get predictions for lapse x dr_severity x any_treatment
  emmeans(fitted_models[[1]], 
          ~ ever_lapse_binary * dr_severity * any_treatment,
          type = "response",  # Get probabilities not log-odds
          weights = "proportional")  # Use observed proportions for other variables
}, error = function(e) {
  # If emmeans fails, try a simpler specification
  emmeans(fitted_models[[1]], 
          ~ ever_lapse_binary + dr_severity + any_treatment,
          type = "response",
...
if (!is.null(emm_results)) {
  ...
  # Create lapse and treatment labels
  pred_df$Lapse <- ifelse(pred_df$ever_lapse_binary == 1, "Lapsed", "Not Lapsed")
  pred_df$Treatment <- ifelse(pred_df$any_treatment == 1, "Treated", "Not Treated")
  
  # Select and reshape for display
  library(tidyr)
  pred_wide <- pred_df |>
  ...
  kable(pred_wide, 
        caption = "Predicted probability of VI/blindness by lapse status (from first imputation)",
        col.names = c("DR Severity", "Treatment", "Not Lapsed", "Lapsed"),
        digits = 3)
  
  # Calculate contrasts for risk differences
  # This properly accounts for the survey design
  contrasts_lapse <- tryCatch({
    pairs(emmeans(fitted_models[[1]], 
                  ~ ever_lapse_binary | dr_severity * any_treatment,
                  type = "response"))
  }, error = function(e) {
    NULL
  ...
  if (!is.null(contrasts_lapse)) {
    contrasts_df <- as.data.frame(contrasts_lapse)
    kable(contrasts_df |> 
          select(dr_severity, any_treatment, estimate, SE, p.value) |>
          mutate(
            Treatment = ifelse(any_treatment == 1, "Treated", "Not Treated"),
            Risk_Difference = round(estimate, 3),
            SE = round(SE, 4),
            p.value = format.pval(p.value, digits = 4, eps = 0.0001)
          ) |>
          select(dr_severity, Treatment, Risk_Difference, SE, p.value),
          caption = "Risk differences (Lapsed - Not Lapsed) with proper variance estimation",
          col.names = c("DR Severity", "Treatment", "Risk Diff", "SE", "P-value"))
  }
} else {
  cat("Note: Predicted probabilities using emmeans could not be calculated.\n")
  cat("The main analysis results above remain valid.\n")
}
```
