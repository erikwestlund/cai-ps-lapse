...
```{r define-formula}
# Get the enhanced formulas for reanalysis from functions.R
reanalysis_formulas <- get_analysis_formulas_reanalysis()

# Use the full formula for main effects analysis
...
if (file.exists(models_cache_file) && !force_refit) {
  ...
  progress <- init_progress(n_imputations, "Fitting models to imputed datasets")
...
convergence_plot <- ggplot(imputation_estimates, aes(x = imputation)) +
  ...
  scale_x_continuous(breaks = scales::pretty_breaks(n = min(10, n_imputations))) +
...
# Add annotation showing stabilization
if (n_imputations >= 10) {
  # Calculate when estimate stabilized (when running avg changes < 1%)
  running_changes <- abs(diff(imputation_estimates$running_avg_OR) / 
                         imputation_estimates$running_avg_OR[-1])
  stabilized_at <- which(running_changes < 0.01)[1]
  
  if (!is.na(stabilized_at) && stabilized_at < n_imputations - 5) {
    convergence_plot <- convergence_plot +
      geom_vline(xintercept = stabilized_at, linetype = "dotted", alpha = 0.5) +
      annotate("text", x = stabilized_at, y = max(imputation_estimates$OR) * 0.98,
               label = paste0("Stabilized (~", stabilized_at, ")"),
               hjust = -0.1, size = 3.5)
  }
}
...
key_vars <- c("ever_lapse_binary", "dr_severityNPDR", "dr_severityPDR")
