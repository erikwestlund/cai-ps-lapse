---
title: "Reanalysis Step 5: Outcome Analysis with Full Four-Treatment Interaction"
subtitle: "IPTW-weighted analysis of lapse × DR severity × 4-level treatment interactions"
...
# Initialize logging
log_file <- init_log("outcome_analysis_full_interaction")

# Configuration for stability checking
MAX_SE_THRESHOLD <- 10  # Maximum acceptable standard error
MIN_N_PER_CELL <- 5    # Minimum observations per interaction cell
TEST_SINGLE_IMPUTATION <- TRUE  # Test on single imputation first

# Treatment categorization approach
# Option 1: "hierarchical" - assigns to highest priority treatment (focal > anti_VEGF > PRP > other)
#           NOTE: focal_laser now has highest priority to preserve all 36 cases
# Option 2: "collapsed" - combines focal_laser with other_inject due to small n
# Option 3: "first_only" - uses only the first/primary treatment
TREATMENT_APPROACH <- "hierarchical"  # Using hierarchical with focal priority
```
...
```{r create-variables}
# Test on single imputation first if configured
if (TEST_SINGLE_IMPUTATION) {
  n_test <- 1
  log_message("TESTING MODE: Will run on single imputation first")
} else {
  n_test <- n_imputations
}
...
for (i in 1:n_imputations) {
  imputed_datasets[[i]] <- imputed_datasets[[i]] |>
    mutate(
      ...
      n_treatments = other_inject + anti_VEGF + focal_laser_flag + PRP_flag,
...
if (sum(first_data$focal_laser_flag) > 0) {
  focal_patients <- first_data[first_data$focal_laser_flag == 1, ]
  ...
  log_message(paste0("  Focal + anti_VEGF: ", sum(focal_patients$anti_VEGF == 1)))
  log_message(paste0("  Focal + PRP: ", sum(focal_patients$PRP_flag == 1)))
  log_message(paste0("  Focal + other: ", sum(focal_patients$other_inject == 1)))
}
...
focal_assignment <- table(
  focal_laser = first_data$focal_laser_flag,
...
## Check Cell Sizes for Full Interaction

```{r check-cells}
# Check cell sizes for the three-way interaction
first_data <- imputed_datasets[[1]]
cell_counts <- first_data |>
  count(ever_lapse_binary, dr_severity, treatment_type) |>
  arrange(n)

# Identify problematic cells
small_cells <- cell_counts |> filter(n < MIN_N_PER_CELL)

if (nrow(small_cells) > 0) {
  log_message(paste0("WARNING: ", nrow(small_cells), 
                    " cells have fewer than ", MIN_N_PER_CELL, " observations"))
  
  print(kable(small_cells, 
              caption = paste("Cells with < ", MIN_N_PER_CELL, " observations")))
  
  # Decide whether to proceed
  if (nrow(small_cells) > length(unique(first_data$treatment_type))) {
    warning("Many small cells detected. Model may be unstable.")
    # Could stop here or collapse categories
  }
}

# Display full interaction table
interaction_table <- cell_counts |>
  pivot_wider(
...
kable(interaction_table,
      caption = "Cell sizes for Lapse × DR Severity × Treatment Type (4 levels)")
```
...
```{r define-formula}
# Define the full interaction formula with 4-level treatment
# This creates a much more complex model than the binary treatment version
full_interaction_formula <- as.formula(
  "outcome_va_vi_binary ~ 
    ever_lapse_binary * dr_severity * treatment_type +  # Full 3-way with 4-level treatment
    baseline_VA_logMAR +
    gender_cat + race_ethnic_cat + insurance_cat + age_cat +
    CCI + DCSI +
...
**Formula:**
```{r show-formula}
print(full_interaction_formula)

# Calculate expected number of terms
# Main effects: 1 (lapse) + 2 (DR) + 4 (treatment) = 7
# Two-way: 1*2 + 1*4 + 2*4 = 2 + 4 + 8 = 14  
# Three-way: 1*2*4 = 8
# Total interaction terms = 7 + 14 + 8 = 29 (plus covariates)
expected_terms <- 29
log_message(paste0("Expected interaction terms: ", expected_terms))
```
...
models_cache_file <- file.path(reanalysis_data_dir, "fitted_models_full_interaction.rds")

# Determine how many models to fit
n_to_fit <- if (TEST_SINGLE_IMPUTATION) 1 else n_imputations

if (file.exists(models_cache_file) && !force_refit_models && !TEST_SINGLE_IMPUTATION) {
  # Load cached models
  ...
  for (i in 1:n_to_fit) {
    log_message(paste0("Fitting model for imputation ", i, " of ", n_to_fit))
    ...
    tryCatch({
      model <- svyglm(
        formula = full_interaction_formula,
      ...
      max_se <- max(coef_summary[, "Std. Error"], na.rm = TRUE)
      
      if (max_se > MAX_SE_THRESHOLD) {
        warning_msg <- paste0("High SE: ", round(max_se, 2))
        problem_terms <- rownames(coef_summary)[coef_summary[, "Std. Error"] > MAX_SE_THRESHOLD]
        log_message(paste0("WARNING: Imputation ", i, " has extreme SEs in: ", 
                          paste(problem_terms, collapse = ", ")))
      }
      
    }, error = function(e) {
      warning_msg <- paste0("Fit failed: ", e$message)
      log_message(paste0("ERROR: Imputation ", i, " - ", warning_msg))
    })
    
    if (fit_success) {
      models[[i]] <- model
      
      # Check if three-way interaction terms exist (with 4-level treatment)
      threeway_terms <- grep("ever_lapse_binary.*:dr_severity.*:treatment_type", 
      ...
      # Stop if testing and issues found
      if (TEST_SINGLE_IMPUTATION && i == 1 && max_se > MAX_SE_THRESHOLD) {
        print(summary(model))
        stop(paste0("Model instability detected. Max SE = ", round(max_se, 2), 
                   ". Consider simplifying model or collapsing treatment categories."))
      }
    }
  }
  ...
  cached_models <- list(
    ...
    formula_used = full_interaction_formula,
