...
for (i in 1:n_imputations) {
  imputed_datasets[[i]] <- imputed_datasets[[i]] |>
    mutate(
      # Create 4-level treatment factor (hierarchical priority)
      # Priority: anti_VEGF > PRP > focal_laser > other_inject > none
      ...
      treatment_type = factor(treatment_type, 
                              levels = c("none", "other_inject", "focal_laser", 
                                       "PRP", "anti_VEGF")),
...
treatment_dist <- table(imputed_datasets[[1]]$treatment_type)
log_message("Treatment type distribution in first imputation:")
