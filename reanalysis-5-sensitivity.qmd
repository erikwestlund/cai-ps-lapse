...
pooled_results <- readRDS(file.path(reanalysis_data_dir, "pooled_results.rds"))
subgroup_effects <- readRDS(file.path(reanalysis_data_dir, "subgroup_effects.rds"))
```
...
message("Main Effect of Lapsing on Vision Impairment:")
message("  Odds Ratio: ", round(or_main, 3))
message("  95% CI: (", round(ci_lower_main, 3), ", ", round(ci_upper_main, 3), ")")
...
evalue_summary_main <- data.frame(
  ...
  E_Value_Point = round(evalue_main$est.Evalue, 3),
  E_Value_CI = round(evalue_main$lower.Evalue, 3)
)
...
# Interpretation
if (evalue_main$est.Evalue > 2) {
  message("\nInterpretation: The E-value of ", round(evalue_main$est.Evalue, 2), 
          " suggests that unmeasured confounding would need to be relatively strong",
          " to explain away the observed association.")
} else if (evalue_main$est.Evalue > 1.5) {
  message("\nInterpretation: The E-value of ", round(evalue_main$est.Evalue, 2),
          " suggests moderate robustness to unmeasured confounding.")
} else {
  message("\nInterpretation: The E-value of ", round(evalue_main$est.Evalue, 2),
          " suggests the finding could be sensitive to unmeasured confounding.")
}
```
...
for (i in 1:nrow(subgroup_effects)) {
  ...
  evalue_results[[i]] <- data.frame(
    ...
    E_Value_Point = round(evalue$est.Evalue, 3),
    E_Value_CI = round(evalue$lower.Evalue, 3)
  )
}
...
message("\nMost robust finding:")
message("  Subgroup: ", most_robust$DR_Severity, " with ", most_robust$Treatment)
message("  E-value: ", most_robust$E_Value_Point)

message("\nLeast robust finding:")
message("  Subgroup: ", least_robust$DR_Severity, " with ", least_robust$Treatment)
message("  E-value: ", least_robust$E_Value_Point)
```
...
message("Benchmarking E-values against measured confounders in our model:")
...
if (nrow(benchmark_effects) > 0) {
  ...
  message("\nStrongest measured confounder OR: ", round(max_measured_or, 2))
  message("Minimum E-value across subgroups: ", round(min_evalue, 2))
  
  if (min_evalue > max_measured_or) {
    message("\nInterpretation: Even the smallest E-value exceeds the strongest",
            " measured confounder, suggesting good robustness.")
  } else {
    message("\nInterpretation: Some E-values are comparable to measured confounders,",
            " suggesting potential sensitivity to unmeasured confounding of similar strength.")
  }
}
```
...
message("E-values under different outcome prevalence assumptions:\n")
...
message("Actual outcome prevalence in data: ", round(outcome_prev * 100, 1), "%")
...
evalue_comparison <- data.frame(
  ...
  E_Value_Point = c(evalue_main$est.Evalue, evalue_common$est.Evalue),
  E_Value_CI = c(evalue_main$lower.Evalue, evalue_common$lower.Evalue)
)
...
if (outcome_prev > 0.1) {
  message("\nNote: Since outcome prevalence is ", round(outcome_prev * 100, 1), 
          "%, the 'Common Outcome' assumption may be more appropriate.")
}
```
...
message("Main Findings:")
message("  - Overall E-value: ", round(evalue_main$est.Evalue, 2))
message("  - Range of subgroup E-values: ", round(min_evalue, 2), " to ", round(max_evalue, 2))
...
message("  - Robust to unmeasured confounding (E > 2): ", robust_subgroups, " subgroups")
message("  - Moderately robust (1.5 < E ≤ 2): ", moderate_subgroups, " subgroups")
message("  - Potentially sensitive (E ≤ 1.5): ", sensitive_subgroups, " subgroups")

# Clinical interpretation
message("\nClinical Interpretation:")
if (min_evalue > 1.5) {
  message("  All subgroup findings show at least moderate robustness to unmeasured confounding.")
} else {
  message("  Some subgroups may be sensitive to unmeasured confounding.")
  message("  Findings should be interpreted with appropriate caution.")
}

# Save E-value results
saveRDS(evalue_table, file.path(reanalysis_data_dir, "evalue_results.rds"))
message("\nE-value results saved to: ", file.path(reanalysis_data_dir, "evalue_results.rds"))
```
