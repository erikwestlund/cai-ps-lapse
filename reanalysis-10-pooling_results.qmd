...
if (length(pooled_results) > 0) {
  cat("\n", paste(rep("=", 60), collapse = ""), "\n")
  cat("POOLING RESULTS SUMMARY\n")
  cat(paste(rep("=", 60), collapse = ""), "\n\n")
  
  # Count successful methods
  n_success <- sum(sapply(pooled_results, function(x) x$success))
  cat("Successfully pooled:", n_success, "of", length(pooled_results), "methods\n\n")
  
  # Show OR range
  or_values <- sapply(pooled_results, function(x) if(x$success) x$or else NA)
  or_values <- or_values[!is.na(or_values)]
  
  if (length(or_values) > 0) {
    cat(sprintf("OR range: %.3f to %.3f\n", min(or_values), max(or_values)))
    cat(sprintf("Median OR: %.3f\n", median(or_values)))
  }
  
  cat("\nForest plot visualization can be generated using:\n")
  cat("  reanalysis-11-forest_plot.qmd\n")
  
  log_message(paste0("Pooling complete. ", n_success, " methods successfully pooled."))
} else {
