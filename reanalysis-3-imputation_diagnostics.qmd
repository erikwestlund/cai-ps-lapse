...
log_message(sprintf("Dataset: %d eyes from %d patients", n_eyes, n_patients))
log_message(sprintf("Bilateral patients: %d (%.1f%%)", n_bilateral, 100*n_bilateral/n_patients))
log_message(sprintf("Missing outcomes: %d eyes (%.1f%%)", n_missing_outcome, 100*n_missing_outcome/n_eyes))
log_message(sprintf("Imputations performed: %d", n_imputations))
```
...
if (!is.na(cor_outcome_imp)) {
  ...
  log_message("Between-eye correlation plot could not be generated.")
}
```
...
## 5. Direction of Change by Missingness Pattern

```{r change-direction-analysis}
# Create detailed groups based on what data was available
direction_df <- imputed_datasets[[1]] |>
  mutate(
    change_baseline_outcome = outcome_VA_logMAR - baseline_VA_logMAR,
    
    # Define groups based on original missingness
    imputation_group = case_when(
      cohort_id == "submitted" ~ "Complete (Observed)",
      cohort_id == "excluded" & !is.na(df_original$intermediate_VA_logMAR[row_number()]) ~ "Imputed (had intermediate)",
      cohort_id == "excluded" & is.na(df_original$intermediate_VA_logMAR[row_number()]) ~ "Imputed (no intermediate)",
      TRUE ~ "Other"
    ),
...
direction_summary <- direction_df |>
  group_by(imputation_group, change_direction) |>
  summarise(n = n(), .groups = "drop") |>
...
# Create table
direction_table <- direction_summary |>
  select(imputation_group, change_direction, n, pct) |>
  pivot_wider(
...
kable(direction_table,
      caption = "Direction of vision change by data availability group") |>
  kable_styling(full_width = FALSE)

# Visualization - stacked bar chart
p_direction <- ggplot(direction_summary, 
                     aes(x = imputation_group, y = pct, fill = factor(change_direction,
  ...
  scale_fill_manual(values = c("Improved" = "#2ca02c", 
                    ...
                    name = "Vision Change") +
  labs(
    title = "Direction of Vision Change by Data Availability",
    subtitle = "Comparing complete vs imputed groups",
    y = "Percentage (%)",
    x = ""
  ) +
  theme_minimal() +
  ...
  ylim(0, max(direction_summary$pct) * 1.15)  # Add space for labels

print(p_direction)

# Chi-square test for independence
chi_table <- table(direction_df$imputation_group, direction_df$change_direction)
chi_test <- chisq.test(chi_table)

log_message(sprintf("Chi-square test: χ² = %.2f, p = %.4f", 
            chi_test$statistic, chi_test$p.value))
if(chi_test$p.value < 0.05) {
  log_message("The proportions of improved/stable/worsened differ significantly across groups.")
} else {
  log_message("The proportions are similar across groups.")
}
```
...
## 7. Multiple Imputation Variance
...
if (cor_preserved) {
  ...
  log_message(sprintf("Between-eye correlation preserved (%.3f baseline → %.3f imputed)", 
              cor_baseline, cor_outcome_imp))
} else {
  cat(sprintf("⚠ Between-eye correlation changed substantially (%.3f → %.3f)\n",
              cor_baseline, cor_outcome_imp))
  log_message(sprintf("WARNING: Between-eye correlation changed substantially (%.3f → %.3f)",
              cor_baseline, cor_outcome_imp))
}
...
if (ks_test$p.value > 0.05) {
  ...
  log_message("Imputed and observed distributions are statistically similar")
} else {
  cat("⚠ Imputed and observed distributions differ significantly\n")
  log_message("WARNING: Imputed and observed distributions differ significantly")
}
...
if (all(effect_sizes$diff_outcome > 0)) {
  ...
  log_message("Lapsed patients have worse outcomes in all cohorts (clinically expected)")
} else {
  cat("⚠ Unexpected lapse-outcome relationship in some cohorts\n")
  log_message("WARNING: Unexpected lapse-outcome relationship in some cohorts")
}

# 4. Temporal consistency
if (all(temporal_summary$mean_discrepancy < 0.001)) {
  cat("✓ Temporal arithmetic is preserved\n")
  log_message("Temporal arithmetic is preserved")
} else {
  cat("⚠ Minor temporal discrepancies detected\n")
  log_message("WARNING: Minor temporal discrepancies detected")
}

# 5. MI stability
if (all(mi_variance$lambda < 0.5)) {
  cat("✓ Fraction of missing information is acceptable (<0.5)\n")
  log_message("Fraction of missing information is acceptable (<0.5)")
} else {
  cat("⚠ High fraction of missing information in some groups\n")
  log_message("WARNING: High fraction of missing information in some groups")
}
...
log_message(sprintf("Excluded cohort has %.1f%% missing outcomes",
            100 - cohort_summary$has_outcome[cohort_summary$cohort_id == "excluded"] / 
            cohort_summary$n_eyes[cohort_summary$cohort_id == "excluded"] * 100))
log_message(sprintf("Lapse rate: excluded cohort (%.1f%%) vs submitted (%.1f%%)",
            cohort_summary$pct_lapsed[cohort_summary$cohort_id == "excluded"],
            cohort_summary$pct_lapsed[cohort_summary$cohort_id == "submitted"]))
...
log_message("Recommendations: 1) 2l.pan accounts for clustering, 2) Consider sensitivity analyses, 3) Report by cohort if needed, 4) Use robust SEs")
