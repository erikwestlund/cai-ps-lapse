...
bilateral_imp <- imputed_datasets[[1]] |>
  ...
  pivot_wider(names_from = eye, values_from = outcome_VA_logMAR)

cor_outcome_imp <- cor(bilateral_imp$outcome_VA_logMAR_eye1, bilateral_imp$outcome_VA_logMAR_eye2, use = "complete.obs")
...
# Visualization
p_bilateral <- ggplot(bilateral_imp, aes(x = outcome_VA_logMAR_eye1, y = outcome_VA_logMAR_eye2)) +
...
# Sample trajectories
set.seed(2025)
sample_ids <- sample(temporal_df$row_number(), min(200, nrow(temporal_df)))
